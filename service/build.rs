...
mod ts {
  ...
  fn op_build_info(state: &mut OpState) -> Value {
    ...
    let node_built_in_module_names = SUPPORTED_BUILTIN_NODE_MODULES.iter().map(|p| p.module_name()).collect::<Vec<&str>>();
  ...
  pub fn create_compiler_snapshot(snapshot_path: PathBuf, cwd: &Path) {
    ...
    println!("cargo:rerun-if-changed={}", cwd.join("src/tsc").join("00_typescript.js").display());
    println!("cargo:rerun-if-changed={}", cwd.join("src/tsc").join("99_main_compiler.js").display());
    println!("cargo:rerun-if-changed={}", cwd.join("src/js").join("40_testing.js").display());
...
fn create_cli_snapshot(snapshot_path: PathBuf) {
  ...
  let extensions: Vec<Extension> = vec![
    ...
    deno_web::deno_web::init_ops::<PermissionsContainer>(deno_web::BlobStore::default(), Default::default()),
