...
impl Emitter {
  pub fn new(emit_cache: EmitCache, parsed_source_cache: Arc<ParsedSourceCache>, emit_options: deno_ast::EmitOptions) -> Self {
    let emit_options_hash = FastInsecureHasher::new().write_hashable(&emit_options).finish();
    Self {
      emit_cache,
      parsed_source_cache,
      emit_options,
  ...
  pub fn maybed_cached_emit(&self, specifier: &ModuleSpecifier, source: &str) -> Option<String> {
