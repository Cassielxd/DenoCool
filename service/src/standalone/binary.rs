  ...
  pub fn into_err(self) -> PackageJsonDepValueParseError {
    match self {
      SerializablePackageJsonDepValueParseError::SchemeValue(value) => {
        PackageJsonDepValueParseError::SchemeValue(PackageDepNpmSchemeValueParseError { value })
      }
      SerializablePackageJsonDepValueParseError::Specifier(source) => PackageJsonDepValueParseError::Specifier(NpmVersionReqSpecifierParseError {
...
#[derive(Serialize, Deserialize)]
pub struct SerializablePackageJsonDeps(BTreeMap<String, Result<NpmPackageReq, SerializablePackageJsonDepValueParseError>>);
...
impl<'a> DenoCompileBinaryWriter<'a> {
  ...
  pub async fn write_bin(
    ...
    let original_binary = self.get_base_binary(compile_flags.target.clone()).await?;

    self
      .write_standalone_binary(writer, original_binary, eszip, module_specifier, cli_options, compile_flags)
      .await
  }

  async fn get_base_binary(&self, target: Option<String>) -> Result<Vec<u8>, AnyError> {
    if target.is_none() {
  ...
  async fn write_standalone_binary(
    ...
    let (npm_snapshot, npm_vfs, npm_files) = if self.npm_resolution.has_packages() {
  ...
  fn build_vfs(&self) -> Result<VfsBuilder, AnyError> {
    if let Some(node_modules_path) = self.npm_resolver.node_modules_path() {
      ...
      for package in self.npm_resolution.all_system_packages(&self.npm_system_info) {
        let folder = self.npm_resolver.resolve_pkg_folder_from_pkg_id(&package.id)?;
