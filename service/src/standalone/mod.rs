...
impl RootCertStoreProvider for StandaloneRootCertStoreProvider {
  fn get_or_try_init(&self) -> Result<&RootCertStore, AnyError> {
    self
      .cell
      .get_or_try_init(|| get_root_cert_store(None, self.ca_stores.clone(), self.ca_data.clone()).map_err(|err| err.into()))
  }
}
...
pub async fn run(eszip: eszip::EszipV2, metadata: Metadata) -> Result<(), AnyError> {
  ...
  let (fs, vfs_root, node_modules_path, snapshot) = if let Some(snapshot) = metadata.npm_snapshot {
  ...
  let npm_resolution = Arc::new(NpmResolution::from_serialized(npm_api.clone(), snapshot, None));
  ...
  let npm_resolver = Arc::new(CliNpmResolver::new(fs.clone(), npm_resolution.clone(), npm_fs_resolver, None));
  ...
  let package_json_deps_provider = Arc::new(PackageJsonDepsProvider::new(
    metadata.package_json_deps.map(|serialized| serialized.into_deps()),
  ));
  ...
  let worker_factory = CliMainWorkerFactory::new(
    ...
    CliMainWorkerOptions {
      ...
      maybe_binary_npm_command_name: NpmPackageReqReference::from_specifier(main_module)
        .ok()
