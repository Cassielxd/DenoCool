...
pub async fn error_handler<F>(watch_future: F)
where
...
async fn next_restart<R, T, F>(resolver: &mut R, debounced_receiver: &mut DebouncedReceiver) -> (Vec<PathBuf>, Result<T, AnyError>)
where
  ...
  F: Future<Output = ResolutionResult<T>>,
{
  loop {
    ...
    match resolver(changed).await {
      ...
      ResolutionResult::Restart { mut paths_to_watch, result } => {
...
fn create_print_after_restart_fn(clear_screen: bool) -> impl Fn() {
  move || {
    ...
    info!("{} File change detected! Restarting!", colors::intense_blue("Watcher"),);
  }
}
...
pub async fn watch_func<R, O, T, F1, F2>(mut resolver: R, mut operation: O, print_config: PrintConfig) -> Result<(), AnyError>
where
  ...
  F2: Future<Output = Result<(), AnyError>>,
{
  ...
  match resolver(None).await {
    ResolutionResult::Ignore => {
      ...
      info!("{} Waiting for file changes...", colors::intense_blue("Watcher"),);
  ...
  loop {
    ...
    match resolution_result {
      Ok(operation_arg) => {
        ...
        info!("{} {} finished. Restarting on file change...", colors::intense_blue("Watcher"), job_name,);
      }
      Err(error) => {
        eprintln!("{}: {}", colors::red_bold("error"), error);
        info!("{} {} failed. Restarting on file change...", colors::intense_blue("Watcher"), job_name,);
      }
    }
