...
pub fn source_map_from_code(code: &ModuleCode) -> Option<Vec<u8>> {
  ...
  if last_line.starts_with(SOURCE_MAP_PREFIX) {
    ...
    let decoded_map = base64::decode(input).expect("Unable to decode source map from emitted file.");
    Some(decoded_map)
  } else {
    None
  }
...
mod tests {
  ...
  fn test_source_without_source_map() {
    ...
    fn run_test(input: &'static str, output: &'static str) {
      assert_eq!(code_without_source_map(ModuleCode::from_static(input)).as_str().to_owned(), output);
    }
  }
}
