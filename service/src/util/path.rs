...
pub fn mapped_specifier_for_tsc(specifier: &ModuleSpecifier, media_type: MediaType) -> Option<String> {
  ...
  if media_type != ext_media_type {
    ...
    if media_type != MediaType::Dts
      ...
      && specifier.path().split('/').last().map(|last| last.contains(".d.")).unwrap_or(false)
    {
...
mod test {
  ...
  fn test_specifier_to_file_path() {
    ...
    run_success_test("file:///dir/test%20test/test.txt", "/dir/test test/test.txt");

    fn run_success_test(specifier: &str, expected_path: &str) {
      let result = specifier_to_file_path(&ModuleSpecifier::parse(specifier).unwrap()).unwrap();
  ...
  fn test_relative_specifier() {
    let fixtures: Vec<(&str, &str, Option<&str>)> = vec![
      ...
      ("file:///from/", "file:///other/from", Some("../other/from")),
      ("file:///from", "file:///other/from/", Some("./other/from/")),
      ("file:///from", "file:///to/other.txt", Some("./to/other.txt")),
      ("file:///from/test", "file:///to/other.txt", Some("../to/other.txt")),
      ("file:///from/other.txt", "file:///to/other.txt", Some("../to/other.txt")),
      ("https://deno.land/x/a/b/d.ts", "https://deno.land/x/a/b/c.ts", Some("./c.ts")),
      ("https://deno.land/x/a/b/d.ts", "https://deno.land/x/a/c.ts", Some("../c.ts")),
      ("https://deno.land/x/a/b/d.ts", "https://deno.land/x/a/b/c/d.ts", Some("./c/d.ts")),
      ("https://deno.land/x/a/b/c/", "https://deno.land/x/a/b/c/d.ts", Some("./d.ts")),
      ("https://deno.land/x/a/b/c/", "https://deno.land/x/a/b/c/d/e.ts", Some("./d/e.ts")),
      ("https://deno.land/x/a/b/c/f.ts", "https://deno.land/x/a/b/c/d/e.ts", Some("./d/e.ts")),
      (
        "https://deno.land/x/a/b/d.ts",
        "https://deno.land/x/a/c.ts?foo=bar",
        Some("../c.ts?foo=bar"),
      ),
      ("https://deno.land/x/a/b/d.ts?foo=bar", "https://deno.land/x/a/b/c.ts", Some("./c.ts")),
      ("file:///a/b/d.ts", "file:///a/b/c.ts", Some("./c.ts")),
      ("https://deno.land/x/a/b/c.ts", "file:///a/b/c.ts", None),
      ("https://deno.land/", "https://deno.land/x/a/b/c.ts", Some("./x/a/b/c.ts")),
      ("https://deno.land/x/d/e/f.ts", "https://deno.land/x/a/b/c.ts", Some("../../a/b/c.ts")),
    ];
    for (from_str, to_str, expected) in fixtures {
      ...
      assert_eq!(actual.as_deref(), expected, "from: \"{from_str}\" to: \"{to_str}\"");
    }
  }
  ...
  fn test_path_with_stem_suffix() {
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/"), "_2"), PathBuf::from("/_2"));
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test"), "_2"), PathBuf::from("/test_2"));
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test.txt"), "_2"), PathBuf::from("/test_2.txt"));
    assert_eq!(
    ...
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test.d.ts"), "_2"), PathBuf::from("/test_2.d.ts"));
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test.D.TS"), "_2"), PathBuf::from("/test_2.D.TS"));
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test.d.mts"), "_2"), PathBuf::from("/test_2.d.mts"));
    assert_eq!(path_with_stem_suffix(&PathBuf::from("/test.d.cts"), "_2"), PathBuf::from("/test_2.d.cts"));
  }
}
