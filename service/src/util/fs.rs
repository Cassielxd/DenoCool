...
pub fn atomic_write_file<T: AsRef<[u8]>>(filename: &Path, data: T, mode: u32) -> std::io::Result<()> {
            ...
            if self.canonicalized_ignore.iter().any(|i| c.starts_with(i)) {
              ...
              let should_ignore_dir = c
                ...
                .map(|dir_name| {
                  ...
                  let is_ignored_file = self.ignore_node_modules && dir_name == "node_modules" || self.ignore_git_folder && dir_name == ".git";
