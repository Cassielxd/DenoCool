...
impl TypeChecker {
  pub fn new(caches: Arc<Caches>, cli_options: Arc<CliOptions>, node_resolver: Arc<NodeResolver>, npm_resolver: Arc<CliNpmResolver>) -> Self {
    Self {
      caches,
      cli_options,
      node_resolver,
...
/// be used to tell
fn get_check_hash(graph: &ModuleGraph, type_check_mode: TypeCheckMode, ts_config: &TsConfig) -> CheckHashResult {
  let mut hasher = FastInsecureHasher::new();
  ...
  for module in sorted_modules {
    match module {
      ...
      Module::Json(_) | Module::External(_) | Module::Node(_) | Module::Npm(_) => {
        // ignore
      }
    }
  }
...
fn has_ts_check(media_type: MediaType, file_text: &str) -> bool {
  match &media_type {
    MediaType::JavaScript | MediaType::Mjs | MediaType::Cjs | MediaType::Jsx => {
      get_leading_comments(file_text).iter().any(|text| TS_CHECK_RE.is_match(text))
    }
    MediaType::TypeScript
    | MediaType::Mts
    | MediaType::Cts
...
mod test {
  ...
  fn get_leading_comments_test() {
    ...
    assert_eq!(get_leading_comments("//1 /* */ \na;"), vec!["1 /* */ ".to_string(),]);
  ...
  fn has_ts_check_test() {
    ...
    assert!(has_ts_check(MediaType::JavaScript, "// deno-lint-ignore\n// @ts-check\n"));
