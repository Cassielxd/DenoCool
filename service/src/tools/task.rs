...
pub async fn execute_script(flags: Flags, task_flags: TaskFlags) -> Result<i32, AnyError> {
  ...
  if let Some(script) = tasks_config.get(task_name) {
    ...
    let package_json_deps_installer = factory.package_json_deps_installer().await?;
    ...
    package_json_deps_installer.ensure_top_level_install().await?;
...
fn resolve_npm_commands(npm_resolver: &CliNpmResolver, node_resolver: &NodeResolver) -> Result<HashMap<String, Rc<dyn ShellCommand>>, AnyError> {
