...
static EXEC_NAME_RE: Lazy<Regex> = Lazy::new(|| RegexBuilder::new(r"^[a-z][\w-]*$").case_insensitive(true).build().expect("invalid regex"));
...
fn get_installer_root() -> Result<PathBuf, io::Error> {
  ...
  let mut home_path = env::var_os(home_env_var)
    .map(PathBuf::from)
    .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, format!("${home_env_var} is not defined")))?;
  home_path.push(".deno");
  Ok(home_path)
}
...
async fn resolve_shim_data(flags: &Flags, install_flags: &InstallFlags) -> Result<ShimData, AnyError> {
    ...
    || NpmPackageReqReference::from_specifier(&module_url).is_ok()
  {
    ...
    if let Some(lock_path) = &flags.lock {
      extra_files.push((
        copy_path,
        fs::read_to_string(lock_path).with_context(|| format!("error reading {}", lock_path.display()))?,
      ));
    } else {
...
fn get_hidden_file_with_ext(file_path: &Path, ext: &str) -> PathBuf {
  ...
  file_path
    .with_file_name(format!(".{}", file_path.file_name().unwrap().to_string_lossy()))
    .with_extension(ext)
}
