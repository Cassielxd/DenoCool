...
pub async fn lint(cli_options: CliOptions, lint_options: LintOptions) -> Result<(), AnyError> {
  ...
  if cli_options.watch_paths().is_some() {
    ...
    if lint_options.is_stdin {
      ...
      handle_lint_result(STDIN_FILE_NAME, r, reporter_lock.clone(), has_error.clone());
...
mod test {
  ...
  fn recommended_rules_when_no_tags_in_config() {
    ...
    let mut rule_names = rules.into_iter().map(|r| r.code().to_string()).collect::<Vec<_>>();
    rule_names.sort();
