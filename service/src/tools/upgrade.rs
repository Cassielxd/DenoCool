...
// How often query service for new version. In hours.
...
async fn fetch_and_store_latest_version<TEnvironment: UpdateCheckerEnvironment>(env: &TEnvironment) {
  // Fetch latest version or commit hash from service.
...
async fn get_latest_release_version(client: &HttpClient) -> Result<String, AnyError> {
  let text = client.download_text("https://dl.deno.land/release-latest.txt").await?;
  let version = text.trim().to_string();
  Ok(version.replace('v', ""))
}

async fn get_latest_canary_version(client: &HttpClient) -> Result<String, AnyError> {
  let text = client.download_text("https://dl.deno.land/canary-latest.txt").await?;
  let version = text.trim().to_string();
  Ok(version)
}
...
pub fn unpack_into_dir(archive_data: Vec<u8>, is_windows: bool, temp_dir: &tempfile::TempDir) -> Result<PathBuf, std::io::Error> {
      ...
      Command::new("powershell.exe")
        ...
        .map_err(|err| {
          if err.kind() == std::io::ErrorKind::NotFound {
            std::io::Error::new(std::io::ErrorKind::NotFound, "`powershell.exe` was not found in your PATH")
          } else {
            err
          }
        })?
...
mod test {
  ...
  fn test_parse_upgrade_check_file() {
    ...
    assert_eq!(file.last_prompt.to_rfc3339(), "2020-01-01T00:00:00+00:00".to_string());
    assert_eq!(file.last_checked.to_rfc3339(), "2020-01-01T00:00:00+00:00".to_string());
  ...
  fn test_serialize_upgrade_check_file() {
    ...
    assert_eq!(file.serialize(), "2020-01-01T00:00:00+00:00!2020-01-01T00:00:00+00:00!1.2.3!1.2.2");
  }
  ...
  impl TestUpdateCheckerEnvironment {
    ...
    pub fn add_hours(&self, hours: i64) {
      ...
      *time = time.checked_add_signed(chrono::Duration::hours(hours)).unwrap();
    }
