...
fn print_cache_info(factory: &CliFactory, json: bool, location: Option<&deno_core::url::Url>) -> Result<(), AnyError> {
  ...
  if json {
    ...
    println!("{} {}", colors::bold("Language service registries cache:"), registry_cache.display(),);
...
fn add_npm_packages_to_json(json: &mut serde_json::Value, npm_resolver: &CliNpmResolver) {
  ...
  fn print_children<TWrite: Write>(writer: &mut TWrite, prefix: &str, children: &Vec<TreeNode>) -> fmt::Result {
    ...
    for (index, child) in children.iter().enumerate() {
      ...
      let child_prefix = format!(
        "{}{}{}",
        prefix,
        if is_last { EMPTY_CONNECTOR } else { VERTICAL_CONNECTOR },
        EMPTY_CONNECTOR
      );
...
impl NpmInfo {
  pub fn build<'a>(graph: &'a ModuleGraph, npm_resolver: &'a CliNpmResolver, npm_snapshot: &'a NpmResolutionSnapshot) -> Self {
  ...
  fn fill_package_info<'a>(&mut self, package: &NpmResolutionPackage, npm_resolver: &'a CliNpmResolver, npm_snapshot: &'a NpmResolutionSnapshot) {
  ...
  pub fn resolve_package(&self, nv: &NpmPackageNv) -> Option<&NpmResolutionPackage> {
...
impl<'a> GraphDisplayContext<'a> {
  pub fn write<TWrite: Write>(graph: &'a ModuleGraph, npm_resolver: &'a CliNpmResolver, writer: &mut TWrite) -> fmt::Result {
  ...
  fn build_error_info(&mut self, err: &ModuleGraphError, specifier: &ModuleSpecifier) -> TreeNode {
    self.seen.insert(specifier.to_string());
    match err {
      ModuleGraphError::ModuleError(err) => match err {
        ModuleError::InvalidTypeAssertion { .. } => self.build_error_msg(specifier, "(invalid import assertion)"),
        ...
        ModuleError::UnsupportedImportAssertionType { .. } => self.build_error_msg(specifier, "(unsupported import assertion)"),
      ...
      ModuleGraphError::ResolutionError(_) => self.build_error_msg(specifier, "(resolution error)"),
    }
  }
