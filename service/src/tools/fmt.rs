...
pub async fn format(cli_options: CliOptions, fmt_options: FmtOptions) -> Result<(), AnyError> {
  ...
  let resolver = |changed: Option<Vec<PathBuf>>| {
    ...
    async move {
      if files_changed && matches!(result, Ok((ref files, _)) if files.is_empty()) {
        ...
        ResolutionResult::Restart { paths_to_watch, result }
      }
    }
  };
  ...
  let operation = |(paths, fmt_options): (Vec<PathBuf>, FmtOptionsConfig)| async {
    let incremental_cache = Arc::new(IncrementalCache::new(caches.fmt_incremental_cache_db(), &fmt_options, &paths));
    if check {
...
fn format_markdown(file_text: &str, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
  ...
  dprint_plugin_markdown::format_text(file_text, &markdown_config, move |tag, text, line_width| {
    ...
    if matches!(
      tag.as_str(),
      "ts" | "tsx" | "js" | "jsx" | "cjs" | "cts" | "mjs" | "mts" | "javascript" | "typescript" | "json" | "jsonc"
    ) {
...
pub fn format_json(file_text: &str, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
  ...
  F: FnOnce(PathBuf) -> Result<(), AnyError> + Send + 'static + Clone,
{
  ...
  let panic_file_paths = join_results
    .iter()
    .enumerate()
    .filter_map(|(i, join_result)| join_result.as_ref().err().map(|_| file_paths[i].to_string_lossy()))
    .collect::<Vec<_>>();
  ...
  let mut errors = join_results
    .into_iter()
    .filter_map(|join_result| join_result.ok().and_then(|handle_result| handle_result.err()));
...
fn is_supported_ext_fmt(path: &Path) -> bool {
  if let Some(ext) = get_extension(path) {
    matches!(
      ext.as_str(),
      "ts" | "tsx" | "js" | "jsx" | "cjs" | "cts" | "mjs" | "mts" | "json" | "jsonc" | "md" | "mkd" | "mkdn" | "mdwn" | "mdown" | "markdown"
    )
  } else {
    false
  }
...
mod test {
  ...
  fn test_format_ensure_stable_unstable_format() {
    format_ensure_stable(&PathBuf::from("mod.ts"), "1", &Default::default(), |_, file_text, _| {
  ...
  fn test_format_ensure_stable_error_first() {
    let err = format_ensure_stable(&PathBuf::from("mod.ts"), "1", &Default::default(), |_, _, _| bail!("Error formatting.")).unwrap_err();
  ...
  fn test_format_stable_after_two() {
    let result = format_ensure_stable(&PathBuf::from("mod.ts"), "1", &Default::default(), |_, file_text, _| {
      if file_text == "1" {
        Ok(Some("11".to_string()))
