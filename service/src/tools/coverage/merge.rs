...
pub fn merge_processes(mut processes: Vec<ProcessCoverage>) -> Option<ProcessCoverage> {
  ...
  for process_cov in processes {
    for script_cov in process_cov.result {
      url_to_scripts.entry(script_cov.url.clone()).or_insert_with(Vec::new).push(script_cov);
    }
  }
...
pub fn merge_scripts(mut scripts: Vec<ScriptCoverage>) -> Option<ScriptCoverage> {
  ...
  let functions: Vec<FunctionCoverage> = range_to_funcs.into_values().map(|funcs| merge_functions(funcs).unwrap()).collect();

  Some(ScriptCoverage { script_id, url, functions })
}
...
fn into_start_events<'a>(trees: Vec<&'a mut RangeTree<'a>>) -> Vec<StartEvent> {
  ...
  for (parent_index, tree) in trees.into_iter().enumerate() {
    for child in tree.children.drain(..) {
      result.entry(child.start).or_insert_with(Vec::new).push((parent_index, child));
    }
  }
...
fn merge_range_tree_children<'a>(rta: &'a RangeTreeArena<'a>, parent_trees: Vec<&'a mut RangeTree<'a>>) -> Vec<&'a mut RangeTree<'a>> {
    ...
    match open_range {
      Some(open_range) => {
        for (parent_index, tree) in event.trees {
          ...
          parent_to_nested.entry(parent_index).or_insert_with(Vec::new).push(child);
        }
      }
      None => {
  ...
  let mut child_forests: Vec<Peekable<::std::vec::IntoIter<&'a mut RangeTree<'a>>>> =
    child_forests.into_iter().map(|forest| forest.into_iter().peekable()).collect();
