...
impl CoverageCollector {
  ...
  async fn start_precise_coverage(&mut self, parameters: StartPreciseCoverageParameters) -> Result<StartPreciseCoverageReturnObject, AnyError> {
  ...
  pub async fn stop_collecting(&mut self) -> Result<(), AnyError> {
    ...
    for script_coverage in script_coverages {
      ...
      let formated_coverage = format_json(&coverage, &Default::default()).ok().flatten().unwrap_or(coverage);
...
fn generate_coverage_report(
  ...
  for line_index in 0..text_lines.lines_count() {
    ...
    let ignore = comment_ranges
      ...
      || script_source[line_start_byte_offset..line_end_byte_offset].trim().is_empty();
    let mut count = 0;
...
fn filter_coverages(coverages: Vec<ScriptCoverage>, include: Vec<String>, exclude: Vec<String>, npm_root_dir: &str) -> Vec<ScriptCoverage> {
...
pub async fn cover_files(flags: Flags, coverage_flags: CoverageFlags) -> Result<(), AnyError> {
  ...
  let script_coverages = filter_coverages(script_coverages, coverage_flags.include, coverage_flags.exclude, root_dir_url.as_str());
