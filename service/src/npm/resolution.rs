...
impl std::fmt::Debug for NpmResolution {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    f.debug_struct("NpmResolution").field("snapshot", &snapshot.as_serialized()).finish()
  }
}
...
impl NpmResolution {
  pub fn from_serialized(
    ...
    let snapshot = NpmResolutionSnapshot::new(NpmResolutionSnapshotCreateOptions {
      api: api.clone(),
      snapshot: initial_snapshot.unwrap_or_default(),
      // WARNING: When bumping this version, check if anything needs to be
      // updated in the `setNodeOnlyGlobalNames` call in 99_main_compiler.js
      types_node_version_req: Some(VersionReq::parse_from_npm("18.0.0 - 18.11.18").unwrap()),
    });
  ...
  pub fn resolve_nv_ref_from_pkg_req_ref(&self, req_ref: &NpmPackageReqReference) -> Result<NpmPackageNvReference, PackageReqNotFoundError> {
  ...
  pub fn resolve_package_cache_folder_id_from_id(&self, id: &NpmPackageId) -> Option<NpmPackageCacheFolderId> {
  ...
  pub fn resolve_pkg_id_from_pkg_req(&self, req: &NpmPackageReq) -> Result<NpmPackageId, PackageReqNotFoundError> {
    self.snapshot.read().resolve_pkg_from_pkg_req(req).map(|pkg| pkg.id.clone())
  }

  pub fn resolve_pkg_id_from_deno_module(&self, id: &NpmPackageNv) -> Result<NpmPackageId, PackageNvNotFoundError> {
  ...
  /// a package.json
  pub fn resolve_package_req_as_pending(&self, pkg_req: &NpmPackageReq) -> Result<NpmPackageNv, NpmPackageVersionResolutionError> {
...
async fn add_package_reqs_to_snapshot(
  ...
  if !snapshot.has_pending() && package_reqs.iter().all(|req| snapshot.package_reqs().contains_key(req)) {
...
fn populate_lockfile_from_snapshot(lockfile: &mut Lockfile, snapshot: &NpmResolutionSnapshot) -> Result<(), AnyError> {
  for (package_req, nv) in snapshot.package_reqs() {
    lockfile.insert_npm_specifier(
      ...
      snapshot.resolve_package_from_deno_module(nv).unwrap().id.as_serialized(),
    );
  }
