...
fn verify_tarball_integrity(package: &NpmPackageNv, data: &[u8], npm_integrity: &str) -> Result<(), AnyError> {
  ...
  let (algo, expected_checksum) = match npm_integrity.split_once('-') {
    ...
    None => bail!("Not implemented integrity kind for {}: {}", package, npm_integrity),
  };
...
mod test {
  ...
  pub fn test_verify_tarball() {
    ...
    assert_eq!(
      verify_tarball_integrity(&package, &Vec::new(), "test").unwrap_err().to_string(),
    ...
    assert_eq!(
      verify_tarball_integrity(&package, &Vec::new(), "sha1-test").unwrap_err().to_string(),
      concat!(
    ...
    assert_eq!(
      verify_tarball_integrity(&package, &Vec::new(), "sha512-test").unwrap_err().to_string(),
      format!("Tarball checksum did not match what was provided by npm registry for package@1.0.0.\n\nExpected: test\nActual: {actual_checksum}"),
    );
    assert!(verify_tarball_integrity(&package, &Vec::new(), &format!("sha512-{actual_checksum}")).is_ok());
  }
}
