...
impl PackageJsonDepsInstallerInner {
  ...
  ) -> FuturesOrdered<impl Future<Output = Result<(&'a NpmPackageReq, Arc<deno_npm::registry::NpmPackageInfo>), NpmRegistryPackageInfoLoadError>>> {
...
impl PackageJsonDepsInstaller {
  ...
  pub async fn ensure_top_level_install(&self) -> Result<(), AnyError> {
    ...
    if package_reqs
      .iter()
      .all(|req| inner.npm_resolution.resolve_pkg_id_from_pkg_req(req).is_ok())
    {
    ...
    while let Some(result) = reqs_with_info_futures.next().await {
      let (req, info) = result?;
      let result = inner.npm_resolution.resolve_package_req_as_pending_with_info(req, &info);
