...
pub async fn cache_packages(mut packages: Vec<NpmResolutionPackage>, cache: &Arc<NpmCache>, registry_url: &Url) -> Result<(), AnyError> {
...
pub fn ensure_registry_read_permission(
  ...
  if path.starts_with(registry_path) && path.components().all(|c| !matches!(c, std::path::Component::ParentDir)) {
...
mod test {
  ...
  fn test_types_package_name() {
    ...
    assert_eq!(types_package_name("@scoped/package"), "@types/@scoped__package");
  }
}
