...
impl CliNpmResolver {
  ...
  pub fn resolve_package_folder_from_specifier(&self, specifier: &ModuleSpecifier) -> Result<PathBuf, AnyError> {
    let path = self.fs_resolver.resolve_package_folder_from_specifier(specifier)?;
    log::debug!("Resolved package folder of {} to {}", specifier, path.display());
    Ok(path)
  }
...
impl NpmResolver for CliNpmResolver {
  ...
  fn in_npm_package(&self, specifier: &ModuleSpecifier) -> bool {
    self.resolve_package_folder_from_specifier(specifier).is_ok()
  }
