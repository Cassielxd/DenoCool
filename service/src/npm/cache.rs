...
pub fn with_folder_sync_lock(package: &NpmPackageNv, output_folder: &Path, action: impl FnOnce() -> Result<(), AnyError>) -> Result<(), AnyError> {
...
impl ReadonlyNpmCache {
  ...
  pub fn package_folder_for_name_and_version(&self, package: &NpmPackageNv, registry_url: &Url) -> PathBuf {
  ...
  fn maybe_resolve_package_folder_id_from_specifier(&self, specifier: &ModuleSpecifier, registry_url: &Url) -> Option<NpmPackageCacheFolderId> {
    let registry_root_dir = self
      .root_dir_url
      .join(&format!(
        "{}/",
        root_url_to_safe_local_dirname(registry_url).to_string_lossy().replace('\\', "/")
      ))
...
impl NpmCache {
  pub fn new(cache_dir_path: PathBuf, cache_setting: CacheSetting, http_client: Arc<HttpClient>, progress_bar: ProgressBar) -> Self {
    Self {
  ...
  pub async fn ensure_package(&self, package: &NpmPackageNv, dist: &NpmPackageVersionDistInfo, registry_url: &Url) -> Result<(), AnyError> {
    self
  ...
  async fn ensure_package_inner(&self, package: &NpmPackageNv, dist: &NpmPackageVersionDistInfo, registry_url: &Url) -> Result<(), AnyError> {
    let package_folder = self.readonly.package_folder_for_name_and_version(package, registry_url);
      ...
      && !package_folder.join(NPM_PACKAGE_SYNC_LOCK_FILENAME).exists()
    {
      ...
      return Err(custom_error(
        "NotCached",
        format!("An npm specifier not found in cache: \"{}\", --cached-only is specified.", &package.name),
      ));
    }
  ...
  pub fn ensure_copy_package(&self, folder_id: &NpmPackageCacheFolderId, registry_url: &Url) -> Result<(), AnyError> {
    ...
    let original_package_folder = self.readonly.package_folder_for_name_and_version(&folder_id.nv, registry_url);
  ...
  pub fn package_folder_for_name_and_version(&self, package: &NpmPackageNv, registry_url: &Url) -> PathBuf {
    self.readonly.package_folder_for_name_and_version(package, registry_url)
  }
  ...
  pub fn resolve_package_folder_id_from_specifier(
    ...
    self.readonly.resolve_package_folder_id_from_specifier(specifier, registry_url)
  }
}
...
mod test {
  ...
  fn should_get_package_folder() {
    ...
    assert_eq!(
      ...
      root_dir.join("registry.npmjs.org").join("json").join("1.2.5"),
    );
    ...
    assert_eq!(
      ...
      root_dir.join("registry.npmjs.org").join("json").join("1.2.5_1"),
    );
    ...
    assert_eq!(
      ...
      root_dir.join("registry.npmjs.org").join("_jjju6tq").join("2.1.5"),
    );
    ...
    assert_eq!(
      ...
      root_dir.join("registry.npmjs.org").join("_ib2hs4dfomxuuu2pjy").join("2.1.5"),
    );
  }
}
