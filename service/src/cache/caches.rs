...
impl Caches {
  ...
  pub fn dep_analysis_db(&self) -> CacheDB {
    Self::make_db(
      ...
      self.dir_provider.get_or_create().ok().map(|dir| dir.dep_analysis_db_file_path()),
    )
  }
  ...
  pub fn type_checking_cache_db(&self) -> CacheDB {
    Self::make_db(
      ...
      &TYPE_CHECK_CACHE_DB,
      self.dir_provider.get_or_create().ok().map(|dir| dir.type_checking_cache_db_file_path()),
    )
  }
}
