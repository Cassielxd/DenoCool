...
impl IncrementalCacheInner {
  pub fn new<TState: Serialize>(db: CacheDB, state: &TState, initial_file_paths: &[PathBuf]) -> Self {
    let state_hash = FastInsecureHasher::new().write_str(&serde_json::to_string(state).unwrap()).finish();
...
impl SqlIncrementalCache {
  ...
  fn get_source_hash_result(&self, path: &Path) -> Result<Option<u64>, AnyError> {
    ...
    let res = self
      .conn
      .query_row(query, params![path.to_string_lossy(), self.state_hash.to_string()], |row| {
  ...
  pub fn set_source_hash(&self, path: &Path, source_hash: u64) -> Result<(), AnyError> {
    ...
    self
      .conn
      .execute(sql, params![path.to_string_lossy(), &self.state_hash.to_string(), &source_hash,])?;
    Ok(())
  }
}
...
mod test {
  ...
  pub async fn incremental_cache_general_use() {
    ...
    let cache = IncrementalCacheInner::from_sql_incremental_cache(sql_cache, &[file_path.clone()]);
