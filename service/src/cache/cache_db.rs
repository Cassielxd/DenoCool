...
impl CacheDB {
  ...
  fn initialize_connection(config: &CacheDBConfiguration, conn: &Connection, version: &str) -> Result<(), AnyError> {
    ...
    let existing_version = conn
      .query_row("SELECT value FROM info WHERE key='CLI_VERSION' LIMIT 1", [], |row| {
        row.get::<_, String>(0)
      })
      .optional()?
      .unwrap_or_default();
  ...
  fn open_connection(&self) -> Result<ConnectionState, AnyError> {
    ...
    match self.config.on_failure {
      CacheFailure::InMemory => {
        log::error!("Failed to open cache file '{}', opening in-memory cache.", path.to_string_lossy());
        Ok(ConnectionState::Connected(self.open_connection_and_init(&None)?))
      }
      CacheFailure::Blackhole => {
        log::error!("Failed to open cache file '{}', performance may be degraded.", path.to_string_lossy());
...
mod tests {
  ...
  async fn simple_database() {
    ...
    db.ensure_connected().expect("Failed to initialize in-memory database");

    db.execute("insert into test values (?1)", [1]).unwrap();
    let res = db
      .query_row("select * from test", [], |row| Ok(row.get::<_, String>(0).unwrap()))
      .unwrap();
  ...
  async fn bad_sql() {
    ...
    db.ensure_connected().expect_err("Expected to fail, but succeeded");
  }

  #[tokio::test]
  async fn failure_mode_in_memory() {
    let db = CacheDB::from_path(&TEST_DB, FAILURE_PATH.into(), "1.0");
    db.ensure_connected().expect("Should have created a database");
  ...
  async fn failure_mode_error() {
    ...
    db.execute("insert into test values (?1)", [1]).expect_err("Should have failed");
    db.query_row("select * from test", [], |row| Ok(row.get::<_, String>(0).unwrap()))
