...
pub static NODE_ANALYSIS_CACHE_DB: CacheDBConfiguration = CacheDBConfiguration {
  ...
  on_version_change: concat!("DELETE FROM cjsanalysiscache;", "DELETE FROM esmglobalscache;",),
  preheat_queries: &[],
...
impl NodeAnalysisCache {
  ...
  pub fn compute_source_hash(text: &str) -> String {
    FastInsecureHasher::new().write_str(text).finish().to_string()
  }
  ...
  pub fn get_esm_analysis(&self, specifier: &str, expected_source_hash: &str) -> Option<Vec<String>> {
    Self::ensure_ok(self.inner.get_esm_analysis(specifier, expected_source_hash))
  }

  pub fn set_esm_analysis(&self, specifier: &str, source_hash: &str, top_level_decls: &Vec<String>) {
    Self::ensure_ok(self.inner.set_esm_analysis(specifier, source_hash, top_level_decls))
  }
}
...
impl NodeAnalysisCacheInner {
  ...
  pub fn set_cjs_analysis(&self, specifier: &str, source_hash: &str, cjs_analysis: &CjsAnalysis) -> Result<(), AnyError> {
    ...
    self
      .conn
      .execute(sql, params![specifier, &source_hash.to_string(), &serde_json::to_string(&cjs_analysis)?,])?;
    Ok(())
  }

  pub fn get_esm_analysis(&self, specifier: &str, expected_source_hash: &str) -> Result<Option<Vec<String>>, AnyError> {
    let query = "
      SELECT
        data
      FROM
  ...
  pub fn set_esm_analysis(&self, specifier: &str, source_hash: &str, top_level_decls: &Vec<String>) -> Result<(), AnyError> {
    let sql = "
      INSERT OR REPLACE INTO
    ...
    self
      .conn
      .execute(sql, params![specifier, &source_hash, &serde_json::to_string(top_level_decls)?,])?;
    Ok(())
  }
}
...
mod test {
  ...
  pub fn node_analysis_cache_general_use() {
    ...
    cache.set_cjs_analysis("file.js", "2", &cjs_analysis).unwrap();
    ...
    cache.set_esm_analysis("file.js", "2", &esm_analysis).unwrap();
