...
impl CliFactoryBuilder {
  ...
  pub fn with_watcher(mut self, sender: tokio::sync::mpsc::UnboundedSender<Vec<PathBuf>>) -> Self {
...
pub struct CliFactory {
  maybe_sender: RefCell<Option<tokio::sync::mpsc::UnboundedSender<Vec<PathBuf>>>>,
...
impl CliFactory {
  ...
  pub fn deno_dir_provider(&self) -> &Arc<DenoDirProvider> {
    self
      .services
      .deno_dir_provider
      .get_or_init(|| Arc::new(DenoDirProvider::new(self.options.maybe_custom_root().clone())))
  }
    ...
    self
      ...
      .get_or_try_init_async(async {
        ...
        Ok(Arc::new(NpmResolution::from_serialized(
          npm_api.clone(),
          self.options.resolve_npm_resolution_snapshot(npm_api).await?,
  ...
  pub async fn create_node_modules_npm_fs_resolver(&self, node_modules_dir_path: PathBuf) -> Result<Arc<dyn NpmPackageFsResolver>, AnyError> {
  ...
  pub fn package_json_deps_provider(&self) -> &Arc<PackageJsonDepsProvider> {
    self
      ...
      .get_or_init(|| Arc::new(PackageJsonDepsProvider::new(self.options.maybe_package_json_deps())))
  }

  pub async fn package_json_deps_installer(&self) -> Result<&Arc<PackageJsonDepsInstaller>, AnyError> {
    self
      .services
  ...
  pub async fn maybe_import_map(&self) -> Result<&Option<Arc<ImportMap>>, AnyError> {
    self
      .services
      .maybe_import_map
      .get_or_try_init_async(async { Ok(self.options.resolve_import_map(self.file_fetcher()?).await?.map(Arc::new)) })
      .await
  }
  ...
  pub fn parsed_source_cache(&self) -> Result<&Arc<ParsedSourceCache>, AnyError> {
    self
      .services
      .parsed_source_cache
      .get_or_try_init(|| Ok(Arc::new(ParsedSourceCache::new(self.caches()?.dep_analysis_db()))))
  }
  ...
  pub async fn node_resolver(&self) -> Result<&Arc<NodeResolver>, AnyError> {
    self
      .services
      .node_resolver
      .get_or_try_init_async(async { Ok(Arc::new(NodeResolver::new(self.fs().clone(), self.npm_resolver().await?.clone()))) })
      .await
  }
  ...
  /// for a file watcher.
  pub async fn create_cli_main_worker_factory_func(&self) -> Result<Arc<dyn Fn() -> CliMainWorkerFactory>, AnyError> {
    ...
    Ok(Arc::new(move || {
      CliMainWorkerFactory::new(
        ...
        Box::new(CliModuleLoaderFactory::new(
          ...
          resolver.clone(),
          NpmModuleLoader::new(cjs_resolutions.clone(), node_code_translator.clone(), fs.clone(), node_resolver.clone()),
        )),
  ...
  fn create_cli_main_worker_options(&self) -> Result<CliMainWorkerOptions, AnyError> {
    Ok(CliMainWorkerOptions {
      ...
      debug: self.options.log_level().map(|l| l == log::Level::Debug).unwrap_or(false),
