...
impl ModuleLoadPreparer {
  ...
  pub async fn prepare_module_load(
    ...
    let maybe_file_watcher_reporter: Option<&dyn deno_graph::source::Reporter> = if let Some(reporter) = &self.maybe_file_watcher_reporter {
      Some(reporter)
    } else {
      None
    };
    ...
    if self.options.type_check_mode() != TypeCheckMode::None && !self.graph_container.is_type_checked(&roots, lib) {
...
impl ModuleLoader for CliModuleLoader {
  fn resolve(&self, specifier: &str, referrer: &str, kind: ResolutionKind) -> Result<ModuleSpecifier, AnyError> {
    ...
    if let Ok(referrer) = referrer_result.as_ref() {
      if let Some(result) = self.shared.npm_module_loader.resolve_if_in_npm_package(specifier, referrer, permissions) {
        return result;
      }
      ...
      match maybe_resolved {
        Some(Resolution::Ok(resolved)) => {
          ...
          return match graph.get(specifier) {
            Some(Module::Npm(module)) => self.shared.npm_module_loader.resolve_nv_ref(&module.nv_reference, permissions),
    ...
    if self.shared.is_repl {
      ...
      if let Some(specifier) = specifier {
        if let Ok(reference) = NpmPackageReqReference::from_specifier(&specifier) {
          return self.shared.npm_module_loader.resolve_req_reference(&reference, permissions);
        }
      }
    }
...
impl NpmModuleLoader {
      ...
      Some(
        self
          .handle_node_resolve_result(
            self
              .node_resolver
              .resolve(specifier, referrer, NodeResolutionMode::Execution, permissions),
          )
  ...
  pub fn resolve_nv_ref(&self, nv_ref: &NpmPackageNvReference, permissions: &PermissionsContainer) -> Result<ModuleSpecifier, AnyError> {
    self
      .handle_node_resolve_result(
        self
          .node_resolver
          .resolve_npm_reference(nv_ref, NodeResolutionMode::Execution, permissions),
      )
      .with_context(|| format!("Could not resolve '{}'.", nv_ref))
  }

  pub fn resolve_req_reference(&self, reference: &NpmPackageReqReference, permissions: &PermissionsContainer) -> Result<ModuleSpecifier, AnyError> {
    self
      .handle_node_resolve_result(
        self
          .node_resolver
          .resolve_npm_req_reference(reference, NodeResolutionMode::Execution, permissions),
      )
  ...
  fn load_sync(
    ...
    let code = self.fs.read_to_string(&file_path).map_err(AnyError::from).with_context(|| {
    ...
    let code = if self.cjs_resolutions.contains(specifier) {
      ...
      self.node_code_translator.translate_cjs_to_esm(specifier, &code, permissions)?
    } else {
      // only inject node globals for esm
      self.node_code_translator.esm_code_with_node_globals(specifier, &code)?
    };
    Ok(ModuleCodeSource {
      code: code.into(),
      found_url: specifier.clone(),
