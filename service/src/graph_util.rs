...
impl ModuleGraphBuilder {
  ...
  pub async fn create_graph_with_loader(&self, roots: Vec<ModuleSpecifier>, loader: &mut dyn Loader) -> Result<deno_graph::ModuleGraph, AnyError> {
    ...
    if graph.has_node_specifier && self.options.type_check_mode() != TypeCheckMode::None {
  ...
  pub async fn create_graph(&self, roots: Vec<ModuleSpecifier>) -> Result<deno_graph::ModuleGraph, AnyError> {
    let mut cache = self.create_graph_loader();
...
pub fn error_for_any_npm_specifier(graph: &ModuleGraph) -> Result<(), AnyError> {
  for module in graph.modules() {
    match module {
      Module::Npm(module) => {
        bail!(
          "npm specifiers have not yet been implemented for this subcommand (https://github.com/denoland/deno/issues/15960). Found: {}",
          module.specifier
        )
      }
      Module::Node(module) => {
...
mod test {
  ...
  fn import_map_node_resolution_error() {
    ...
    for (input, output) in cases {
      let import_map = import_map::ImportMap::new(ModuleSpecifier::parse("file:///deno.json").unwrap());
