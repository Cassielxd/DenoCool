...
declare namespace Deno {
  ...
  export interface Proxy {
    /** The string URL of the proxy service to use. */
    url: string;
    /** The basic auth credentials to be used against the proxy service. */
    basicAuth?: BasicAuth;
  }
   ...
   * service when specifying {@linkcode Deno.CreateHttpClientOptions}.
   *
   * @category Fetch API
   */
  export interface BasicAuth {
    /** The username to be used against the proxy service. */
    username: string;
    /** The password to be used against the proxy service. */
    password: string;
  }
  ...
  export interface TlsHandshakeInfo {
    /** **UNSTABLE**: New API, yet to be vetted.
     *
     * Contains the ALPN protocol selected during negotiation with the service.
  ...
  export interface TlsConn extends Conn {
    /** **UNSTABLE**: New API, yet to be vetted.
     *
     * Runs the client or service handshake protocol to completion if that has
   ...
   *
   * If a handler throws, the service calling the handler will assume the impact
  ...
  export interface ServeOptions extends Partial<Deno.ListenOptions> {
    /** An {@linkcode AbortSignal} to close the service and all connections. */
    signal?: AbortSignal;
    ...
    /** The callback which is called when the service starts listening. */
    onListen?: (params: { hostname: string; port: number }) => void;
  }

  /** **UNSTABLE**: New API, yet to be vetted.
   *
   * Additional options which are used when opening a TLS (HTTPS) service.
   *
   * @category HTTP Server
   */
  ...
  export interface Server {
    /** A promise that resolves once service finishes - eg. when aborted using
     * the signal passed to {@linkcode ServeOptions.signal}.
     */
    finished: Promise<void>;

    /**
     * Make the service block the event loop from finishing.
     *
     * Note: the service blocks the event loop from finishing by default.
     * This method is only meaningful after `.unref()` is called.
     */
    ref(): void;

    /** Make the service not block the event loop from finishing. */
    unref(): void;
  }
   ...
   * ```
   *
   * You can stop the service with an {@linkcode AbortSignal}. The abort signal
   * needs to be passed as the `signal` option in the options bag. The service
   * aborts when the abort signal is aborted. To wait for the service to close,
   ...
   * );
   * service.finished.then(() => console.log("Server closed"));
   *
   ...
   *     // ... more info specific to your service ..
   *   },
   ...
   * {@linkcode Deno.serveHttp} service.
   *
   * @category HTTP Server
   */
