...
impl FileFetcher {
  ...
  fn fetch_data_url(&self, specifier: &ModuleSpecifier) -> Result<File, AnyError> {
    ...
    self.http_cache.set(specifier, headers.clone(), source.as_bytes())?;
    ...
    match &self.cache_setting {
      ...
      CacheSetting::RespectHeaders => {
        if let Ok((_, headers, cache_time)) = self.http_cache.get(specifier) {
          let cache_semantics = CacheSemantics::new(headers, cache_time, SystemTime::now());
  ...
  pub async fn fetch_with_accept(
    ...
    if let Some(file) = self.cache.get(specifier) {
      ...
      let result = self.fetch_remote(specifier, permissions, 10, maybe_accept.map(String::from)).await;
      if let Ok(file) = &result {
        self.cache.insert(specifier.clone(), file.clone());
      }
      result
