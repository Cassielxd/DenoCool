...
/// variable.
fn get_completor_type(offset: usize, tokens: &[Token], match_result: &MatchResult) -> Option<CompletorType> {
  let mut len = 0_usize;
...
fn get_endpoint_with_match(
  ...
  for (key, value) in match_result.params.iter() {
    if let StringOrNumber::String(name) = key {
      ...
      url = url.replace(
        &format!("${{{{{name}}}}}"),
        &percent_encoding::percent_encode(value.to_string(maybe_key, true).as_bytes(), COMPONENT).to_string(),
      );
    }
  }
  if !has_value {
...
pub struct RegistryConfigurationVariable {
  ...
  /// completion item when requested by the language service.
...
impl ModuleRegistry {
  pub fn new(location: &Path, http_client: Arc<HttpClient>) -> Self {
    ...
    let mut file_fetcher = FileFetcher::new(http_cache, CacheSetting::RespectHeaders, true, http_client, BlobStore::default(), None);
  ...
  async fn fetch_config(&self, specifier: &ModuleSpecifier) -> Result<Vec<RegistryConfiguration>, AnyError> {
    ...
    if fetch_result.is_err() {
      ...
      self.file_fetcher.http_cache.set(specifier, headers_map, &[])?;
    }
        ...
        if let Some(registries) = self.origins.get(&origin) {
          ...
          for registry in registries {
            ...
            loop {
              ...
              if let Some(match_result) = matcher.matches(path) {
                did_match = true;
                let completor_type = get_completor_type(path_offset, &tokens, &match_result);
                match completor_type {
                  Some(CompletorType::Literal(s)) => self.complete_literal(s, &mut completions, current_specifier, offset, range),
