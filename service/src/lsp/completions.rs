...
async fn check_auto_config_registry(url_str: &str, config: &ConfigSnapshot, client: &Client, module_registries: &ModuleRegistry) {
...
pub async fn get_import_completions(
  ...
  if let Some(completion_list) = get_import_map_completions(specifier, &text, &range, maybe_import_map.clone(), documents) {
    ...
    let maybe_list = module_registries
      .get_completions(&text, offset, &range, |specifier| documents.exists(specifier))
      .await;
