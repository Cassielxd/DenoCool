...
/// to allow the Deno language service to manage these as virtual documents.
    ...
    if let Some(url) = inner.get_url(specifier).cloned() {
      ...
      let url = if specifier.scheme() == "file" {
        ...
        let specifier_str = if specifier.scheme() == "asset" {
          ...
          let mut path = specifier[..Position::BeforePath].replacen("://", "/", 1);
          let parts: Vec<String> = specifier[Position::BeforePath..]
            .split('/')
            .map(|p| percent_encoding::utf8_percent_encode(p, COMPONENT).to_string())
            .collect();
...
mod tests {
  ...
  fn test_hash_data_specifier() {
    let fixture =
      resolve_url("data:application/typescript;base64,ZXhwb3J0IGNvbnN0IGEgPSAiYSI7CgpleHBvcnQgZW51bSBBIHsKICBBLAogIEIsCiAgQywKfQo=").unwrap();
    let actual = hash_data_specifier(&fixture);
    assert_eq!(actual, "c21c7fc382b2b0553dc0864aa81a3acacfb7b3d1285ab5ae76da6abec213fb37");
  }
  ...
  fn test_lsp_url_map() {
    ...
    let actual_url = map.normalize_specifier(&fixture).expect("could not handle specifier");
    let expected_url = Url::parse("deno:/https/deno.land/x/pkg%401.0.0/mod.ts").unwrap();
    assert_eq!(actual_url.as_url(), &expected_url);

    let actual_specifier = map.normalize_url(actual_url.as_url(), LspUrlKind::File);
  ...
  fn test_lsp_url_map_complex_encoding() {
    ...
    let actual_url = map.normalize_specifier(&fixture).expect("could not handle specifier");
    let expected_url =
      Url::parse("deno:/https/cdn.skypack.dev/-/postcss%40v8.2.9-E4SktPp9c0AtxrJHp8iV/dist%3Des2020%2Cmode%3Dtypes/lib/postcss.d.ts").unwrap();
    assert_eq!(actual_url.as_url(), &expected_url);
  ...
  fn test_normalize_windows_path() {
    let map = LspUrlMap::default();
    let fixture = resolve_url("file:///c%3A/Users/deno/Desktop/file%20with%20spaces%20in%20name.txt").unwrap();
    let actual = map.normalize_url(&fixture, LspUrlKind::File);
    let expected = Url::parse("file:///C:/Users/deno/Desktop/file with spaces in name.txt").unwrap();
  ...
  fn test_normalize_percent_encoded_path() {
    let map = LspUrlMap::default();
    let fixture = resolve_url("file:///Users/deno/Desktop/file%20with%20spaces%20in%20name.txt").unwrap();
    let actual = map.normalize_url(&fixture, LspUrlKind::File);
    let expected = Url::parse("file:///Users/deno/Desktop/file with spaces in name.txt").unwrap();
    assert_eq!(actual, expected);
  }
