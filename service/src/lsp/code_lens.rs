...
pub async fn collect(
  ...
  code_lenses.extend(collect_tsc(specifier, config.workspace_settings(), line_index, navigation_tree).await?);

  Ok(code_lenses)
}

fn collect_test(specifier: &ModuleSpecifier, parsed_source: Option<ParsedSource>, config: &Config) -> Result<Vec<lsp::CodeLens>, AnyError> {
  ...
  fn test_deno_test_collector() {
    ...
    assert_eq!(
      ...
      vec![
        ...
        lsp::CodeLens {
          range: lsp::Range {
            start: lsp::Position { line: 6, character: 11 },
            end: lsp::Position { line: 6, character: 15 }
          },
          command: Some(lsp::Command {
            title: "▶\u{fe0e} Run Test".to_string(),
            command: "deno.test".to_string(),
        ...
        lsp::CodeLens {
          range: lsp::Range {
            start: lsp::Position { line: 8, character: 11 },
            end: lsp::Position { line: 8, character: 15 }
          },
          command: Some(lsp::Command {
            title: "▶\u{fe0e} Run Test".to_string(),
            command: "deno.test".to_string(),
