...
impl TestServer {
  pub fn new(client: Client, performance: Arc<Performance>, maybe_root_uri: Option<ModuleSpecifier>) -> Self {
    let tests: Arc<Mutex<HashMap<ModuleSpecifier, TestDefinitions>>> = Arc::new(Mutex::new(HashMap::new()));
              ...
              for document in snapshot.documents.documents(DocumentsFilter::AllDiagnosable) {
                ...
                if !valid {
                  if let Some(Ok(parsed_source)) = document.maybe_parsed_source() {
                    let mut collector = TestCollector::new(specifier.clone(), parsed_source.text_info().clone());
                    ...
                    if !test_definitions.discovered.is_empty() {
                      client.send_test_notification(test_definitions.as_notification(specifier, mru.as_ref(), parsed_source.text_info()));
                    }
