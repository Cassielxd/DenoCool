...
) -> (HashSet<ModuleSpecifier>, HashMap<ModuleSpecifier, LspTestFilter>) {
...
impl LspTestFilter {
  fn as_ids(&self, test_definitions: &TestDefinitions) -> Vec<String> {
    let ids: Vec<String> = if let Some(include) = &self.include {
      ...
      test_definitions.discovered.iter().map(|td| td.id.clone()).collect()
    };
...
impl TestRun {
      ...
      .map(|s| {
        let ids = if let Some(test_definitions) = tests.get(s) {
          if let Some(filter) = self.filters.get(s) {
            ...
            test_definitions.discovered.iter().map(|test| test.id.clone()).collect()
          }
        } else {
          Vec::new()
        };
  ...
  pub async fn exec(&self, client: &Client, maybe_root_uri: Option<&ModuleSpecifier>) -> Result<(), AnyError> {
    ...
    let (concurrent_jobs, fail_fast) = if let DenoSubcommand::Test(test_flags) = factory.cli_options().sub_command() {
      (
        test_flags.concurrent_jobs.unwrap_or_else(|| NonZeroUsize::new(1).unwrap()).into(),
...
impl From<&TestOrTestStepDescription> for lsp_custom::TestIdentifier {
  fn from(desc: &TestOrTestStepDescription) -> lsp_custom::TestIdentifier {
    match desc {
      ...
      TestOrTestStepDescription::TestStepDescription(test_step_desc) => test_step_desc.into(),
    }
  }
}
...
impl From<&test::TestStepDescription> for lsp_custom::TestIdentifier {
  fn from(desc: &test::TestStepDescription) -> Self {
    ...
    Self {
      text_document: lsp::TextDocumentIdentifier { uri },
      id: Some(checksum::gen(&[desc.origin.as_bytes(), desc.root_name.as_bytes()])),
...
impl LspTestReporter {
  ...
  fn report_register(&mut self, desc: &test::TestDescription) {
    ...
    let tds = tests.entry(ModuleSpecifier::parse(&desc.location.file_name).unwrap()).or_default();
    if tds.inject(desc.into()) {
      ...
      self
        .client
        .send_test_notification(TestingNotification::Module(lsp_custom::TestModuleNotificationParams {
  ...
  fn report_output(&mut self, output: &[u8]) {
    let test = self
      .current_origin
      .as_ref()
      .and_then(|origin| self.stack.get(origin).and_then(|v| v.last().map(|td| td.into())));
  ...
  fn report_result(&mut self, desc: &test::TestDescription, result: &test::TestResult, elapsed: u64) {
    ...
    match result {
      test::TestResult::Ok => self.progress(lsp_custom::TestRunProgressMessage::Passed {
        test: desc.into(),
        duration: Some(elapsed as u32),
      }),
      test::TestResult::Ignored => self.progress(lsp_custom::TestRunProgressMessage::Skipped { test: desc.into() }),
      test::TestResult::Failed(failure) => self.progress(lsp_custom::TestRunProgressMessage::Failed {
        test: desc.into(),
        messages: as_test_messages(failure.to_string(), false),
        duration: Some(elapsed as u32),
      }),
      test::TestResult::Cancelled => self.progress(lsp_custom::TestRunProgressMessage::Failed {
        test: desc.into(),
        messages: vec![],
        duration: Some(elapsed as u32),
      }),
  ...
  fn report_step_result(&mut self, desc: &test::TestStepDescription, result: &test::TestStepResult, elapsed: u64) {
    ...
    match result {
      ...
      test::TestStepResult::Ignored => self.progress(lsp_custom::TestRunProgressMessage::Skipped { test: desc.into() }),
...
mod tests {
  ...
  fn test_as_queue_and_filters() {
    ...
    exclude.insert("69d9fe87f64f5b66cb8b631d4fd2064e8224b8715a049be54276c42189ff8f9f".to_string(), test_def_b);
