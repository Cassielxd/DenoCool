...
impl TestDefinition {
  pub fn new(specifier: &ModuleSpecifier, name: String, range: SourceRange, steps: Vec<TestDefinition>) -> Self {
  ...
  pub fn new_step(name: String, range: SourceRange, parent: String, level: usize, steps: Vec<TestDefinition>) -> Self {
    let id = checksum::gen(&[parent.as_bytes(), &level.to_be_bytes(), name.as_bytes()]);
    Self {
      id,
      level,
      name,
  ...
  fn as_test_data(&self, source_text_info: &SourceTextInfo) -> lsp_custom::TestData {
    lsp_custom::TestData {
      id: self.id.clone(),
      label: self.name.clone(),
      steps: self.steps.iter().map(|step| step.as_test_data(source_text_info)).collect(),
...
impl TestDefinitions {
  ...
  pub fn as_notification(
    ...
    let mut tests_map: HashMap<String, lsp_custom::TestData> = self.injected.iter().map(|td| (td.id.clone(), td.clone())).collect();
  ...
  pub fn inject(&mut self, data: lsp_custom::TestData) -> bool {
    if self.discovered.iter().any(|td| td.contains_id(&data.id)) || self.injected.iter().any(|td| td.id == data.id) {
      return false;
    }
  ...
  pub fn get_by_id<S: AsRef<str>>(&self, id: S) -> Option<&TestDefinition> {
    self.discovered.iter().find(|td| td.id.as_str() == id.as_ref())
  }
}
