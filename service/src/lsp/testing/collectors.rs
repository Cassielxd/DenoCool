...
fn arrow_to_steps(parent: &str, level: usize, arrow_expr: &ast::ArrowExpr) -> Vec<TestDefinition> {
  if let Some((maybe_test_context, maybe_step_var)) = parse_test_context_param(arrow_expr.params.get(0)) {
    let mut collector = TestStepCollector::new(parent.to_string(), level, maybe_test_context, maybe_step_var);
...
fn fn_to_steps(parent: &str, level: usize, function: &ast::Function) -> Vec<TestDefinition> {
  ...
  if let Some(expr) = node.args.get(0).map(|es| es.expr.as_ref()) {
    match expr {
      ...
      ast::Expr::Ident(ident) => {
        ...
        fns.and_then(|fns| fns.get(&name).map(|fn_expr| (name, fn_to_steps(parent, level, fn_expr))))
      }
      _ => {
        if let Some(text_info) = text_info {
          let range = node.range();
          let indexes = text_info.line_and_column_display(range.start);
          Some((format!("Test {}:{}", indexes.line_number, indexes.column_number), vec![]))
        } else {
          None
        }
      }
...
impl TestStepCollector {
  fn new(parent: String, level: usize, maybe_test_context: Option<String>, maybe_step_var: Option<String>) -> Self {
  ...
  fn add_step<N: AsRef<str>>(&mut self, name: N, range: SourceRange, steps: Vec<TestDefinition>) {
    let step = TestDefinition::new_step(name.as_ref().to_string(), range, self.parent.clone(), self.level, steps);
    self.steps.push(step);
  }

  fn check_call_expr(&mut self, node: &ast::CallExpr, range: SourceRange) {
    if let Some((name, steps)) = check_call_expr(&self.parent, node, self.level + 1, None, None) {
...
impl TestCollector {
  ...
  fn add_definition<N: AsRef<str>>(&mut self, name: N, range: SourceRange, steps: Vec<TestDefinition>) {
    let definition = TestDefinition::new(&self.specifier, name.as_ref().to_string(), range, steps);
    self.definitions.push(definition);
  }

  fn check_call_expr(&mut self, node: &ast::CallExpr, range: SourceRange) {
    if let Some((name, steps)) = check_call_expr(self.specifier.as_str(), node, 1, Some(&self.fns), Some(&self.text_info)) {
...
pub mod tests {
  ...
  pub fn new_range(start: usize, end: usize) -> SourceRange {
    SourceRange::new(StartSourcePos::START_SOURCE_POS + start, StartSourcePos::START_SOURCE_POS + end)
  }
  ...
  fn test_test_collector_a() {
    ...
    assert_eq!(
      ...
      vec![TestDefinition {
        ...
        range: new_range(12, 16),
        steps: vec![TestDefinition {
          id: "b3b2daad49e5c3095fe26aba0a840131f3d8f32e105e95507f5fc5118642b059".to_string(),
          level: 1,
          name: "step".to_string(),
