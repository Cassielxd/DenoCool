...
impl CliGraphResolver {
  pub fn new(
    ...
    Self {
      ...
      maybe_default_jsx_import_source: maybe_jsx_import_source_config.as_ref().and_then(|c| c.default_specifier.clone()),
      maybe_jsx_import_source_module: maybe_jsx_import_source_config.map(|c| c.module),
      no_npm,
      npm_registry_api,
      npm_resolution,
...
impl Resolver for CliGraphResolver {
  ...
  fn resolve(&self, specifier: &str, referrer: &ModuleSpecifier) -> Result<ModuleSpecifier, AnyError> {
    use MappedResolution::*;
    match self.mapped_specifier_resolver.resolve(specifier, referrer)? {
...
impl NpmResolver for CliGraphResolver {
  ...
  fn load_and_cache_npm_package_info(&self, package_name: &str) -> LocalBoxFuture<'static, Result<(), AnyError>> {
    ...
    async move {
      ...
      let result = api.package_info(&package_name).await.map(|_| ()).map_err(|err| err.into());
      drop(permit);
      result
    }
    .boxed()
  ...
  fn resolve_npm(&self, package_req: &NpmPackageReq) -> NpmPackageReqResolution {
    if self.no_npm {
      return NpmPackageReqResolution::Err(anyhow!("npm specifiers were requested; but --no-npm is specified"));
    }

    let result = self.npm_resolution.resolve_package_req_as_pending(package_req);
    match result {
...
mod test {
  ...
  fn test_resolve_package_json_dep() {
    fn resolve(specifier: &str, deps: &BTreeMap<String, NpmPackageReq>) -> Result<Option<String>, String> {
      let deps = deps.iter().map(|(key, value)| (key.to_string(), Ok(value.clone()))).collect();
    ...
    let deps = BTreeMap::from([
      ("package".to_string(), NpmPackageReq::from_str("package@1.0").unwrap()),
      ("package-alias".to_string(), NpmPackageReq::from_str("package@^1.2").unwrap()),
      ("@deno/test".to_string(), NpmPackageReq::from_str("@deno/test@~0.2").unwrap()),
    ]);

    assert_eq!(resolve("package", &deps).unwrap(), Some("npm:package@1.0".to_string()),);
    assert_eq!(
      resolve("package/some_path.ts", &deps).unwrap(),
      Some("npm:package@1.0/some_path.ts".to_string()),
    );

    assert_eq!(resolve("@deno/test", &deps).unwrap(), Some("npm:@deno/test@~0.2".to_string()),);
    assert_eq!(
    ...
    // alias
    assert_eq!(resolve("package-alias", &deps).unwrap(), Some("npm:package@^1.2".to_string()),);
