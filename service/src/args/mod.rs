...
impl FmtOptions {
  pub fn resolve(maybe_fmt_config: Option<FmtConfig>, mut maybe_fmt_flags: Option<FmtFlags>) -> Result<Self, AnyError> {
...
impl LintOptions {
  pub fn resolve(maybe_lint_config: Option<LintConfig>, mut maybe_lint_flags: Option<LintFlags>) -> Result<Self, AnyError> {
    ...
    let (maybe_file_flags, maybe_rules_tags, maybe_rules_include, maybe_rules_exclude) = maybe_lint_flags
      .map(|f| (f.files, f.maybe_rules_tags, f.maybe_rules_include, f.maybe_rules_exclude))
      .unwrap_or_default();
...
fn discover_package_json(flags: &Flags, maybe_stop_at: Option<PathBuf>, current_dir: &Path) -> Result<Option<PackageJson>, AnyError> {
  ...
  if let Some(package_json_dir) = flags.package_json_search_dir(current_dir) {
    let package_json_dir = canonicalize_path_maybe_not_exists(&package_json_dir)?;
...
impl CliOptions {
  pub fn new(
    ...
    let maybe_node_modules_folder = resolve_local_node_modules_folder(&initial_cwd, &flags, maybe_config_file.as_ref(), maybe_package_json.as_ref())
  ...
  pub fn from_flags(flags: Flags) -> Result<Self, AnyError> {
    ...
    if flags.config_flag == ConfigFlag::Disabled || flags.no_npm || has_flag_env_var("DENO_NO_PACKAGE_JSON") {
  ...
  pub async fn resolve_npm_resolution_snapshot(&self, api: &CliNpmRegistryApi) -> Result<Option<ValidSerializedNpmResolutionSnapshot>, AnyError> {
      ...
      if !lockfile.lock().overwrite {
        return Ok(Some(
          snapshot_from_lockfile(lockfile.clone(), api)
            .await
            .with_context(|| format!("failed reading lockfile '{}'", lockfile.lock().filename.display()))?,
        ));
      }
    }
  ...
  pub fn node_modules_dir_enablement(&self) -> Option<bool> {
    self
      .flags
      .node_modules_dir
      .or_else(|| self.maybe_config_file.as_ref().and_then(|c| c.node_modules_dir()))
  }
  ...
  pub fn resolve_ts_config_for_emit(&self, config_type: TsConfigType) -> Result<TsConfigForEmit, AnyError> {
    config_file::get_ts_config_for_emit(config_type, self.maybe_config_file.as_ref())
  }
  ...
  pub fn to_maybe_jsx_import_source_config(&self) -> Option<JsxImportSourceConfig> {
    self.maybe_config_file.as_ref().and_then(|c| c.to_maybe_jsx_import_source_config())
  }
...
fn resolve_local_node_modules_folder(
  ...
  let use_node_modules_dir = flags.node_modules_dir.or_else(|| maybe_config_file.and_then(|c| c.node_modules_dir()));
...
fn resolve_import_map_specifier(
  ...
  if let Some(import_map_path) = maybe_import_map_path {
    ...
    let specifier =
      deno_core::resolve_url_or_path(import_map_path, current_dir).with_context(|| format!("Bad URL (\"{import_map_path}\") for import map."))?;
    ...
    if config_file.is_an_import_map() {
      if let Some(_import_map_path) = config_file.to_import_map_path() {
        log::warn!(
          "{} \"importMap\" setting is ignored when \"imports\" or \"scopes\" are specified in the config file.",
          colors::yellow("Warning")
        );
      }
...
pub fn npm_pkg_req_ref_to_binary_command(req_ref: &NpmPackageReqReference) -> String {
  let binary_name = req_ref.sub_path.as_deref().unwrap_or(req_ref.req.name.as_str());
