...
pub async fn snapshot_from_lockfile(
  ...
  let (root_packages, mut packages) = {
    ...
    let mut root_packages = HashMap::<NpmPackageReq, NpmPackageId>::with_capacity(lockfile.content.npm.specifiers.len());
    // collect the specifiers to version mappings
    for (key, value) in &lockfile.content.npm.specifiers {
      let package_req = NpmPackageReq::from_str(key).with_context(|| format!("Unable to parse npm specifier: {key}"))?;
  ...
  SerializedNpmResolutionSnapshot { packages, root_packages }
    .into_valid()
