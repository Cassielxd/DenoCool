...
#[op]
pub fn op_pledge_test_permissions(state: &mut OpState, args: ChildPermissionsArg) -> Result<Uuid, AnyError> {
...
#[op(v8)]
fn op_register_test<'a>(scope: &mut v8::HandleScope<'a>, state: &mut OpState, info: TestInfo<'a>) -> Result<TestRegisterResult, AnyError> {
...
#[op]
fn op_register_test_step(state: &mut OpState, info: TestStepInfo) -> Result<TestRegisterResult, AnyError> {
  let id = NEXT_ID.fetch_add(1, Ordering::SeqCst);
  let origin = state.borrow::<ModuleSpecifier>().to_string();
