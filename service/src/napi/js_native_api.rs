...
fn napi_create_dataview(env: *mut Env, len: usize, data: *mut *mut u8, byte_offset: usize, result: *mut napi_value) -> napi_status {
  ...
  let value = data_view
    .new_instance(&mut env.scope(), &[value.into(), byte_offset.into(), byte_length.into()])
    .unwrap();
  let value: v8::Local<v8::Value> = value.into();
  *result = value.into();
  napi_ok
...
fn napi_create_external_arraybuffer(
  ...
  let store: UniqueRef<BackingStore> = transmute(v8__ArrayBuffer__NewBackingStore__with_data(
    data,
    byte_length,
...
#[napi_sym::napi_sym]
fn napi_close_escapable_handle_scope(env: *mut Env, _scope: napi_escapable_handle_scope) -> napi_status {
...
fn napi_define_properties(env_ptr: *mut Env, obj: napi_value, property_count: usize, properties: *const napi_property_descriptor) -> napi_status {
  ...
  for property in properties {
    ...
    if property.getter.is_some() || property.setter.is_some() {
      ...
      return_status_if_false!(env_ptr, !define_maybe.unwrap_or(false), napi_invalid_arg);
