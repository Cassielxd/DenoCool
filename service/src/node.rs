...
impl CjsEsmCodeAnalyzer for CliCjsEsmCodeAnalyzer {
  fn analyze_cjs(&self, specifier: &ModuleSpecifier, source: &str) -> Result<ExtNodeCjsAnalysis, AnyError> {
  ...
  fn analyze_esm_top_level_decls(&self, specifier: &ModuleSpecifier, source: &str) -> Result<HashSet<String>, AnyError> {
    ...
    let source_hash = NodeAnalysisCache::compute_source_hash(source);
    if let Some(decls) = self.cache.get_esm_analysis(specifier.as_str(), &source_hash) {
      ...
      self
        .cache
        .set_esm_analysis(specifier.as_str(), &source_hash, &top_level_decls.clone().into_iter().collect::<Vec<_>>());
      Ok(top_level_decls)
    }
  }
}
