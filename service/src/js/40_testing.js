...
const OP_DETAILS = {
  ...
  "op_net_connect_tcp": ["connect to a TCP service", "awaiting a `Deno.connect` call"],
  "op_net_connect_unix": ["connect to a Unix service", "awaiting a `Deno.connect` call"],
  ...
  "op_net_connect_tls": ["connect to a TLS service", "awaiting a `Deno.connectTls` call"],
...
function assertOps(fn) {
  ...
  return async function asyncOpSanitizer(desc) {
    ...
    // We're checking diff because one might spawn HTTP service in the background
