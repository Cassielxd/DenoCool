...
// Usage: provide a port as argument to run hyper_hello benchmark service
...
pub fn root_path() -> PathBuf {
  PathBuf::from(concat!(env!("CARGO_MANIFEST_DIR"))).parent().unwrap().to_path_buf()
}
...
fn ensure_test_server_built() {
  // if the test service doesn't exist then remind the developer to build first
  if !test_server_path().exists() {
    panic!("Test service not found. Please cargo build before running the tests.");
  }
}

/// Benchmark service that just serves "hello world" responses.
...
fn redirect_resp(url: String) -> Response<Body> {
  ...
  *redirect_resp.status_mut() = StatusCode::MOVED_PERMANENTLY;
  redirect_resp
    .headers_mut()
    .insert(hyper::header::LOCATION, HeaderValue::from_str(&url[..]).unwrap());
...
async fn auth_redirect(req: Request<Body>) -> hyper::Result<Response<Body>> {
  if let Some(auth) = req.headers().get("authorization").map(|v| v.to_str().unwrap()) {
...
async fn echo_websocket_handler(ws: fastwebsockets::WebSocket<Upgraded>) -> Result<(), anyhow::Error> {
...
type WsHandler = fn(fastwebsockets::WebSocket<Upgraded>) -> Pin<Box<dyn Future<Output = Result<(), anyhow::Error>> + Send>>;
  ...
  S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
  let srv_fn = service_fn(move |mut req: Request<Body>| async move {
    let (response, upgrade_fut) = fastwebsockets::upgrade::upgrade(&mut req).map_err(|e| anyhow!("Error upgrading websocket connection: {}", e))?;
  ...
  tokio::spawn(async move {
    let conn_fut = hyper::server::conn::Http::new().serve_connection(stream, srv_fn).with_upgrades();

    if let Err(e) = conn_fut.await {
      eprintln!("websocket service error: {e:?}");
    }
  });
}
...
async fn ping_websocket_handler(ws: fastwebsockets::WebSocket<Upgraded>) -> Result<(), anyhow::Error> {
  ...
  for i in 0..9 {
    ws.write_frame(Frame::new(true, OpCode::Ping, None, vec![])).await.unwrap();

    let frame = ws.read_frame().await.unwrap();
    assert_eq!(frame.opcode, OpCode::Pong);
    assert!(frame.payload.is_empty());

    ws.write_frame(Frame::text(format!("hello {}", i).as_bytes().to_vec())).await.unwrap();
  ...
  ws.write_frame(fastwebsockets::Frame::close(1000, b"")).await.unwrap();
...
async fn close_websocket_handler(ws: fastwebsockets::WebSocket<Upgraded>) -> Result<(), anyhow::Error> {
  let mut ws = fastwebsockets::FragmentCollector::new(ws);

  ws.write_frame(fastwebsockets::Frame::close_raw(vec![])).await.unwrap();
...
async fn get_tls_config(cert: &str, key: &str, ca: &str, http_versions: SupportedHttpVersions) -> io::Result<Arc<rustls::ServerConfig>> {
  ...
  let certs: Vec<Certificate> = {
    ...
    rustls_pemfile::certs(&mut cert_reader).unwrap().into_iter().map(Certificate).collect()
  };

  let mut ca_cert_reader = io::BufReader::new(ca_file);
  let ca_cert = rustls_pemfile::certs(&mut ca_cert_reader).expect("Cannot load CA certificate").remove(0);

  let mut key_reader = io::BufReader::new(key_file);
  let key = {
    let pkcs8_key = rustls_pemfile::pkcs8_private_keys(&mut key_reader).expect("Cannot load key file");
    let rsa_key = rustls_pemfile::rsa_private_keys(&mut key_reader).expect("Cannot load key file");
  ...
  match key {
    Some(key) => {
      ...
      let mut config = rustls::ServerConfig::builder()
        .with_safe_defaults()
        .with_client_cert_verifier(Arc::new(rustls::server::AllowAnyAnonymousOrAuthenticatedClient::new(root_cert_store)))
...
async fn run_wss_server(addr: &SocketAddr) {
  ...
  let tls_config = get_tls_config(cert_file, key_file, ca_cert_file, Default::default()).await.unwrap();
...
/// This service responds with 'PASS' if client authentication was successful. Try it by running
/// test_server and
...
async fn main_server(req: Request<Body>) -> Result<Response<Body>, hyper::http::Error> {
  return match (req.method(), req.uri().path()) {
    ...
    (&hyper::Method::POST, "/echo_multipart_file") => {
      ...
      response
        .headers_mut()
        .insert("content-type", HeaderValue::from_static("multipart/form-data;boundary=boundary"));
      Ok(response)
    }
    ...
    (_, "/multipart_form_bad_content_type") => {
      ...
      res
        .headers_mut()
        .insert("content-type", HeaderValue::from_static("multipart/form-datatststs;boundary=boundary"));
      Ok(res)
    }
    (_, "/bad_redirect") => {
    ...
    (_, "/x_deno_warning.js") => {
      ...
      *res.status_mut() = StatusCode::MOVED_PERMANENTLY;
      res.headers_mut().insert("X-Deno-Warning", HeaderValue::from_static("foobar"));
      res
        .headers_mut()
        .insert("location", HeaderValue::from_bytes(b"/lsp/x_deno_warning_redirect.js").unwrap());
      Ok(res)
    }
    (_, "/non_ascii_redirect") => {
      let mut res = Response::new(Body::empty());
      *res.status_mut() = StatusCode::MOVED_PERMANENTLY;
      res.headers_mut().insert("location", HeaderValue::from_bytes(b"/redirect\xae").unwrap());
      Ok(res)
    }
    (_, "/etag_script.ts") => {
      ...
      if if_none_match == Some(&HeaderValue::from_static("33a64df551425fcc55e")) {
        ...
        *resp.status_mut() = StatusCode::NOT_MODIFIED;
        resp
          .headers_mut()
          .insert("Content-type", HeaderValue::from_static("application/typescript"));
        resp.headers_mut().insert("ETag", HeaderValue::from_static("33a64df551425fcc55e"));
    ...
    (_, "/xTypeScriptTypes.js") => {
      ...
      res
        .headers_mut()
        .insert("Content-type", HeaderValue::from_static("application/javascript"));
      res
        .headers_mut()
        .insert("X-TypeScript-Types", HeaderValue::from_static("./xTypeScriptTypes.d.ts"));
      Ok(res)
    }
    (_, "/xTypeScriptTypes.jsx") => {
      let mut res = Response::new(Body::from("export const foo = 'foo';"));
      res.headers_mut().insert("Content-type", HeaderValue::from_static("text/jsx"));
      res
        .headers_mut()
        .insert("X-TypeScript-Types", HeaderValue::from_static("./xTypeScriptTypes.d.ts"));
      Ok(res)
    ...
    (_, "/.well-known/deno-import-intellisense.json") => {
      ...
      if let Ok(body) = tokio::fs::read(file_path).await {
        Ok(custom_headers("/.well-known/deno-import-intellisense.json", body))
      } else {
    ...
    (_, "/dynamic") => {
      let mut res = Response::new(Body::from(serde_json::to_string_pretty(&std::time::SystemTime::now()).unwrap()));
    ...
    (_, "/dynamic_cache") => {
      ...
      res
        .headers_mut()
        .insert("cache-control", HeaderValue::from_static("public, max-age=604800, immutable"));
      Ok(res)
    }
    (_, "/dynamic_module.ts") => {
    ...
    (_, "/echo_accept") => {
      ...
      let res = Response::new(Body::from(serde_json::json!({ "accept": accept }).to_string()));
      Ok(res)
    }
    _ => {
      let mut file_path = testdata_path();
      ...
      if let Some(suffix) = req.uri().path().strip_prefix("/npm/registry/@denotest/") {
        ...
        if let Ok(file) = tokio::fs::read(&file_path).await {
          ...
          if let Err(err) = download_npm_registry_file(req.uri(), &file_path, is_tarball).await {
            return Response::builder()
              .status(StatusCode::INTERNAL_SERVER_ERROR)
              .body(format!("{err:#}").into());
          };
...
fn handle_custom_npm_registry_path(path: &str) -> Result<Option<Response<Body>>, anyhow::Error> {
  ...
  if parts.len() == 2 {
    if let Some(file_bytes) = cache.tarball_bytes(&package_name, parts[1].trim_end_matches(".tgz"))? {
...
async fn download_npm_registry_file(uri: &hyper::Uri, file_path: &PathBuf, is_tarball: bool) -> Result<(), anyhow::Error> {
  let url_parts = uri.path().strip_prefix("/npm/registry/").unwrap().split('/').collect::<Vec<_>>();
...
struct HyperAcceptor<'a> {
  acceptor: Pin<Box<dyn Stream<Item = io::Result<tokio_rustls::server::TlsStream<TcpStream>>> + 'a>>,
}
...
async fn wrap_double_redirect_server() {
  let double_redirects_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(double_redirects)) });
...
async fn wrap_inf_redirect_server() {
  let inf_redirects_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(inf_redirects)) });
...
async fn wrap_another_redirect_server() {
  let another_redirect_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(another_redirect)) });
...
async fn wrap_auth_redirect_server() {
  let auth_redirect_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(auth_redirect)) });
...
async fn wrap_basic_auth_redirect_server() {
  let basic_auth_redirect_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(basic_auth_redirect)) });
  let basic_auth_redirect_addr = SocketAddr::from(([127, 0, 0, 1], BASIC_AUTH_REDIRECT_PORT));
  let basic_auth_redirect_server = Server::bind(&basic_auth_redirect_addr).serve(basic_auth_redirect_svc);
...
async fn wrap_abs_redirect_server() {
  let abs_redirect_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(absolute_redirect)) });
...
async fn wrap_main_server() {
  let main_server_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(main_server)) });
...
async fn wrap_main_https_server() {
  ...
  let tls_config = get_tls_config(cert_file, key_file, ca_cert_file, Default::default()).await.unwrap();
  loop {
    let tcp = TcpListener::bind(&main_server_https_addr).await.expect("Cannot bind TCP");
    ...
    let main_server_https_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(main_server)) });
    let main_server_https = Server::builder(HyperAcceptor {
      acceptor: incoming_tls_stream,
    })
    .serve(main_server_https_svc);

    //continue to prevent TLS error stopping the service
...
async fn wrap_https_h1_only_tls_server() {
  ...
  let key_file = "tls/localhost.key";
  let ca_cert_file = "tls/RootCA.pem";
  let tls_config = get_tls_config(cert_file, key_file, ca_cert_file, SupportedHttpVersions::Http1Only)
    .await
    .unwrap();
  loop {
    let tcp = TcpListener::bind(&main_server_https_addr).await.expect("Cannot bind TCP");
...
async fn wrap_https_h2_only_tls_server() {
  ...
  let key_file = "tls/localhost.key";
  let ca_cert_file = "tls/RootCA.pem";
  let tls_config = get_tls_config(cert_file, key_file, ca_cert_file, SupportedHttpVersions::Http2Only)
    .await
    .unwrap();
  loop {
    let tcp = TcpListener::bind(&main_server_https_addr).await.expect("Cannot bind TCP");
...
async fn wrap_https_h1_only_server() {
  ...
  let main_server_http_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(main_server)) });
  let main_server_http = Server::bind(&main_server_http_addr).http1_only(true).serve(main_server_http_svc);
...
async fn wrap_https_h2_only_server() {
  ...
  let main_server_http_svc = make_service_fn(|_| async { Ok::<_, Infallible>(service_fn(main_server)) });
  let main_server_http = Server::bind(&main_server_http_addr).http2_only(true).serve(main_server_http_svc);
  let _ = main_server_http.await;
}
...
// Use the single-threaded scheduler. The hyper service is used as a point of
...
fn custom_headers(p: &str, body: Vec<u8>) -> Response<Body> {
  ...
  if p.ends_with("/run/import_compression/brotli") {
    response.headers_mut().insert("Content-Encoding", HeaderValue::from_static("br"));
    response
      .headers_mut()
      .insert("Content-Type", HeaderValue::from_static("application/javascript"));
    response.headers_mut().insert("Content-Length", HeaderValue::from_static("26"));
    return response;
  }
  if p.ends_with("/run/import_compression/gziped") {
    ...
    response
      .headers_mut()
      .insert("Content-Type", HeaderValue::from_static("application/javascript"));
    response.headers_mut().insert("Content-Length", HeaderValue::from_static("39"));
    return response;
  }
  ...
  if p.contains("/encoding/") {
    ...
    response.headers_mut().insert(
      "Content-Type",
      HeaderValue::from_str(&format!("application/typescript;charset={charset}")[..]).unwrap(),
    );
    return response;
  }
...
pub fn run_and_collect_output(
  ...
  run_and_collect_output_with_args(expect_success, args.split_whitespace().collect(), input, envs, need_http_server)
}
...
pub fn run_powershell_script_file(script_file_path: &str, args: Vec<&str>) -> std::result::Result<(), i64> {
  let deno_dir = new_deno_dir();
...
pub fn parse_strace_output(output: &str) -> HashMap<String, StraceOutput> {
  ...
  let mut lines = output
    .lines()
    .filter(|line| !line.is_empty() && !line.contains("detached ...") && !line.contains("unfinished ...") && !line.contains("????"));
...
grault",
    ...
    assert!(pattern_match(multiline_pattern, &multi_line_builder("baz", Some("QUX=quux")), wildcard));

    // Correct input & no leading line
    assert!(pattern_match(multiline_pattern, &multi_line_builder("baz", None), wildcard));
    ...
    assert!(!pattern_match(multiline_pattern, &multi_line_builder("garply", None), wildcard));
  }
