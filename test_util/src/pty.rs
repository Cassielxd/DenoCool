...
impl Pty {
  pub fn new(program: &Path, args: &[&str], cwd: &Path, env_vars: Option<HashMap<String, String>>) -> Self {
  ...
  pub fn read_until(&mut self, end_text: impl AsRef<str>) -> String {
    self.read_until_with_advancing(|text| text.find(end_text.as_ref()).map(|index| index + end_text.as_ref().len()))
  }
  ...
  #[track_caller]
  fn read_until_with_advancing(&mut self, mut condition: impl FnMut(&str) -> Option<usize>) -> String {
  ...
  fn read_until_condition(&mut self, mut condition: impl FnMut(&mut Self) -> bool) {
    let timeout_time = Instant::now().checked_add(Duration::from_secs(5)).unwrap();
...
#[cfg(unix)]
fn create_pty(program: &Path, args: &[&str], cwd: &Path, env_vars: Option<HashMap<String, String>>) -> Box<dyn SystemPty> {
...
mod windows {
  ...
  impl WinPseudoConsole {
    pub fn new(program: &Path, args: &[&str], cwd: &Path, maybe_env_vars: Option<HashMap<String, String>>) -> Self {
  ...
  impl ProcThreadAttributeList {
    pub fn new(console_handle: HPCON) -> Self {
      ...
      unsafe {
        ...
        assert_eq!(InitializeProcThreadAttributeList(ptr::null_mut(), attribute_count, 0, &mut size), FALSE);
  ...
  fn create_pipe() -> (WinHandle, WinHandle) {
    ...
    assert_win_success!(unsafe { CreatePipe(&mut read_handle, &mut write_handle, ptr::null_mut(), 0) });
  ...
  fn to_windows_str(str: &str) -> Vec<u16> {
    ...
    std::ffi::OsStr::new(str).encode_wide().chain(Some(0)).collect()
  }
  ...
  fn get_env_vars(env_vars: HashMap<String, String>) -> Vec<u16> {
    ...
    format!("{}\0", parts.join("")).encode_utf16().collect::<Vec<_>>()
  }
}
