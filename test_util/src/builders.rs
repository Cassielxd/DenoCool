...
impl TestContextBuilder {
  ...
  pub fn env(mut self, key: impl AsRef<str>, value: impl AsRef<str>) -> Self {
    self.envs.insert(key.as_ref().to_string(), value.as_ref().to_string());
    self
  }
  ...
  pub fn build(&self) -> TestContext {
    ...
    let temp_dir = if self.use_separate_deno_dir { TempDir::new() } else { deno_dir.clone() };
    ...
    let http_server_guard = if self.use_http_server { Some(Rc::new(http_server())) } else { None };
...
impl TestCommandBuilder {
  ...
  fn build_args(&self) -> Vec<String> {
    if self.args_vec.is_empty() {
      std::borrow::Cow::Owned(self.args.split_whitespace().map(|s| s.to_string()).collect::<Vec<_>>())
    } else {
      assert!(self.args.is_empty(), "Do not provide args when providing args_vec.");
      std::borrow::Cow::Borrowed(&self.args_vec)
    }
    .iter()
    .map(|arg| arg.replace("$TESTDATA", &self.context.testdata_dir.to_string_lossy()))
    .collect::<Vec<_>>()
  }
  ...
  pub fn with_pty(&self, mut action: impl FnMut(Pty)) {
    ...
    action(Pty::new(&self.build_command_path(), &args, &self.build_cwd(), Some(envs)))
  }
  ...
  pub fn run(&self) -> TestCommandOutput {
    ...
    let combined = combined_reader.map(|pipe| sanitize_output(read_pipe_to_string(pipe), &args));
...
impl Drop for TestCommandOutput {
  ...
  fn drop(&mut self) {
    ...
    if !*self.asserted_exit_code.borrow() && self.exit_code != Some(0) {
      panic!("The non-zero exit code of the command was not asserted: {:?}", self.exit_code,)
    }
  }
}
...
impl TestCommandOutput {
  ...
  pub fn combined_output(&self) -> &str {
    ...
    self.combined.as_deref().expect("not available since .split_output() was called")
  }
  ...
  pub fn assert_exit_code(&self, expected_exit_code: i32) -> &Self {
    ...
    if let Some(exit_code) = &actual_exit_code {
      if *exit_code != expected_exit_code {
        self.print_output();
        panic!("bad exit code, expected: {:?}, actual: {:?}", expected_exit_code, exit_code,);
      }
    } else {
      self.print_output();
  ...
  #[track_caller]
  pub fn assert_stderrr_matches_file(&self, file_path: impl AsRef<Path>) -> &Self {
  ...
  fn inner_assert_matches_file(&self, actual: &str, file_path: impl AsRef<Path>) -> &Self {
    ...
    let expected_text = std::fs::read_to_string(&output_path).unwrap_or_else(|err| panic!("failed loading {}\n\n{err:#}", output_path.display()));
