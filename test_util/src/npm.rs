...
pub static CUSTOM_NPM_PACKAGE_CACHE: Lazy<CustomNpmPackageCache> = Lazy::new(CustomNpmPackageCache::default);
...
impl CustomNpmPackageCache {
  pub fn tarball_bytes(&self, name: &str, version: &str) -> Result<Option<Vec<u8>>> {
    Ok(self.get_package_property(name, |p| p.tarballs.get(version).cloned())?.flatten())
  }
  ...
  fn get_package_property<TResult>(&self, package_name: &str, func: impl FnOnce(&CustomNpmPackage) -> TResult) -> Result<Option<TResult>> {
...
fn get_npm_package(package_name: &str) -> Result<Option<CustomNpmPackage>> {
  ...
  for entry in fs::read_dir(&package_folder)? {
    ...
    let mut tarball_bytes = Vec::new();
    {
      let mut encoder = GzEncoder::new(&mut tarball_bytes, Compression::default());
      {
        ...
        builder
          ...
          .with_context(|| format!("Error adding tarball for directory: {}", version_folder.display()))?;
        builder.finish()?;
      }
      encoder.finish()?;
    }
    ...
    dist.insert("integrity".to_string(), format!("sha512-{tarball_checksum}").into());
    ...
    let package_json_text =
      fs::read_to_string(&package_json_path).with_context(|| format!("Error reading package.json at {}", package_json_path.display()))?;
    let mut version_info: serde_json::Map<String, serde_json::Value> = serde_json::from_str(&package_json_text)?;
      ...
      if let Some(optional_deps) = maybe_optional_deps.as_object() {
        if let Some(maybe_deps) = version_info.get("dependencies") {
          if let Some(deps) = maybe_deps.as_object() {
            ...
            version_info.insert("dependencies".to_string(), serde_json::to_value(cloned_deps).unwrap());
          }
        } else {
          version_info.insert("dependencies".to_string(), serde_json::to_value(optional_deps).unwrap());
        }
      }
    }
