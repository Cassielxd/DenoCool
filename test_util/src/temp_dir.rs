...
impl TempDir {
  ...
  fn new_inner(parent_dir: &Path, prefix: Option<&str>) -> Self {
    ...
    let dir = builder.tempdir_in(parent_dir).expect("Failed to create a temporary directory");
    Self(dir.into())
  }
  ...
  pub fn symlink_dir(&self, oldpath: impl AsRef<Path>, newpath: impl AsRef<Path>) {
    ...
    #[cfg(not(unix))]
    {
      ...
      symlink_dir(self.path().join(oldpath), self.path().join(newpath)).unwrap();
    }
  }
  ...
  pub fn symlink_file(&self, oldpath: impl AsRef<Path>, newpath: impl AsRef<Path>) {
    ...
    #[cfg(not(unix))]
    {
      ...
      symlink_file(self.path().join(oldpath), self.path().join(newpath)).unwrap();
    }
  }
}
