...
pub fn check_test_glob(base: &'static str, glob_pattern: &'static str, files: &[(&'static str, &'static str)]) {
  ...
  for file in glob(&format!("{}/{}", base, glob_pattern)).expect("Failed to read test path") {
    let mut file = file.expect("Invalid file from glob").canonicalize().unwrap();
    file.set_extension("");
    let name = file.file_name().unwrap().to_string_lossy();
    // Strip windows slashes
    let file = file.to_string_lossy().replace('\\', "/");
    let file = file.strip_prefix(&base_dir).expect("File {file} did not start with {base_dir} prefix");
  ...
  for (test, path) in files {
    ...
    let path = if path.is_empty() { (*test).to_owned() } else { path.replace(' ', "") };
  ...
  if error || !found.is_empty() {
    panic!("Glob did not match provided list of files. Expected: \n[\n  {}\n]", list.join(",\n  "));
  }
}
