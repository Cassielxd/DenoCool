...
static CONTENT_TYPE_REG: Lazy<Regex> = lazy_regex::lazy_regex!(r"(?i)^content-length:\s+(\d+)");
...
impl LspStdoutReader {
  ...
  pub fn read_latest_message<R>(&mut self, mut get_match: impl FnMut(&LspMessage) -> Option<R>) -> R {
    let (msg_queue, cvar) = &*self.pending_messages;
    let mut msg_queue = msg_queue.lock();
    loop {
...
impl InitializeParamsBuilder {
  ...
  pub fn set_workspace_folders(&mut self, folders: Vec<lsp_types::WorkspaceFolder>) -> &mut Self {
  ...
  pub fn disable_testing_api(&mut self) -> &mut Self {
    let obj = self.params.capabilities.experimental.as_mut().unwrap().as_object_mut().unwrap();
  ...
  pub fn set_code_lens(&mut self, value: Option<serde_json::Value>) -> &mut Self {
    let options = self.initialization_options_mut();
  ...
  pub fn set_tls_certificate(&mut self, value: impl AsRef<str>) -> &mut Self {
    let options = self.initialization_options_mut();
    options.insert("tlsCertificate".to_string(), value.as_ref().to_string().into());
    self
  }
  ...
  pub fn add_test_server_suggestions(&mut self) -> &mut Self {
    self.set_suggest_imports_hosts(vec![("http://localhost:4545/".to_string(), true)])
  }

  pub fn set_suggest_imports_hosts(&mut self, values: Vec<(String, bool)>) -> &mut Self {
    let options = self.initialization_options_mut();
  ...
  pub fn with_capabilities(&mut self, mut action: impl FnMut(&mut ClientCapabilities)) -> &mut Self {
    action(&mut self.params.capabilities);
    self
  }

  fn initialization_options_mut(&mut self) -> &mut serde_json::Map<String, serde_json::Value> {
...
impl LspClientBuilder {
  ...
  /// between the test client and service.
  ...
  pub fn build_result(&self) -> Result<LspClient> {
    ...
    Ok(LspClient {
      ...
      request_id: 1,
      start: Instant::now(),
      context: self.context.clone().unwrap_or_else(|| TestContextBuilder::new().build()),
      writer,
      deno_dir,
    })
  }
...
impl LspClient {
  ...
  pub fn initialize(&mut self, do_build: impl Fn(&mut InitializeParamsBuilder)) {
  ...
  pub fn initialize_with_config(&mut self, do_build: impl Fn(&mut InitializeParamsBuilder), config: Value) {
  ...
  pub fn did_open_with_config(&mut self, params: Value, config: Value) -> CollectedDiagnostics {
  ...
  fn get_latest_diagnostic_batch_index(&mut self) -> usize {
    let result = self.write_request("deno/internalLatestDiagnosticBatchIndex", json!(null));
  ...
  pub fn read_diagnostics(&mut self) -> CollectedDiagnostics {
    // ask the service what the latest diagnostic batch index is
    let latest_diagnostic_batch_index = self.get_latest_diagnostic_batch_index();
    ...
    while read < 3 {
      let (method, response) = self.read_notification::<DiagnosticBatchNotificationParams>();
    ...
    for _ in 0..total_messages_len {
      let (method, response) = self.read_latest_notification::<lsp::PublishDiagnosticsParams>();
  ...
  pub fn read_notification_with_method<R>(&mut self, expected_method: &str) -> Option<R>
  where
    R: de::DeserializeOwned,
  {
    self.reader.read_message(|msg| match msg {
  ...
  {
    self.reader.read_message(|msg| match msg {
      LspMessage::Request(id, method, maybe_params) => {
        Some((*id, method.to_owned(), maybe_params.clone().map(|p| serde_json::from_value(p).unwrap())))
      }
      _ => None,
    })
  }
  ...
  pub fn get_completion(&mut self, uri: impl AsRef<str>, position: (usize, usize), context: Value) -> lsp::CompletionResponse {
  ...
  pub fn get_completion_list(&mut self, uri: impl AsRef<str>, position: (usize, usize), context: Value) -> lsp::CompletionList {
  ...
  pub fn write_request_with_res_as<R>(&mut self, method: impl AsRef<str>, params: impl Serialize) -> R
  where
    R: de::DeserializeOwned,
  {
  ...
  pub fn write_request(&mut self, method: impl AsRef<str>, params: impl Serialize) -> Value {
...
impl CollectedDiagnostics {
  ...
  pub fn messages_with_source(&self, source: &str) -> lsp::PublishDiagnosticsParams {
    self
      .all_messages()
      .iter()
  ...
  pub fn messages_with_file_and_source(&self, specifier: &str, source: &str) -> lsp::PublishDiagnosticsParams {
    let specifier = Url::parse(specifier).unwrap();
    self
      .all_messages()
      .iter()
      .find(|p| p.uri == specifier && p.diagnostics.iter().any(|d| d.source == Some(source.to_string())))
      .map(ToOwned::to_owned)
      .unwrap()
  }
}
