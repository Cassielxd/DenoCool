...
pub trait WebSocketPermissions {
  fn check_net_url(&mut self, _url: &url::Url, _api_name: &str) -> Result<(), AnyError>;
}
...
{
  state.borrow_mut::<WP>().check_net_url(&url::Url::parse(&url)?, &api_name)?;

  if cancel_handle {
    let rid = state.resource_table.add(WsCancelResource(CancelHandle::new_rc()));
    Ok(Some(rid))
  } else {
    Ok(None)
  }
...
async fn handshake<S: AsyncRead + AsyncWrite + Send + Unpin + 'static>(
  ...
  let client = fastwebsockets::handshake::client(&LocalExecutor, request, socket);
  ...
  .map_err(|err| DomExceptionNetworkError::new(&format!("failed to connect to WebSocket: {err}")))?;

  let upgraded = upgraded.into_inner();
  let stream = WebSocketStream::new(stream::WsStreamKind::Upgraded(upgraded), None);
...
{
  {
    ...
    s.borrow_mut::<WP>()
      ...
      .expect("Permission check should have been done in op_ws_check_permission");
  }

  let cancel_resource = if let Some(cancel_rid) = cancel_handle {
    let r = state.borrow_mut().resource_table.get::<WsCancelResource>(cancel_rid)?;
    Some(r.0.clone())
  } else {
    None
  };

  let unsafely_ignore_certificate_errors = state.borrow().try_borrow::<UnsafelyIgnoreCertificateErrors>().and_then(|it| it.0.clone());
  let root_cert_store = state.borrow().borrow::<WsRootStoreProvider>().get_or_try_init()?;
  let user_agent = state.borrow().borrow::<WsUserAgent>().0.clone();
  let uri: Uri = url.parse()?;
  let mut request = Request::builder()
    .method(Method::GET)
    .uri(uri.path_and_query().ok_or(type_error("Missing path in url".to_string()))?.as_str());

  let authority = uri.authority().unwrap().as_str();
  let host = authority.find('@').map(|idx| authority.split_at(idx + 1).1).unwrap_or_else(|| authority);
  request = request
  ...
  let (stream, response) = match uri.scheme_str() {
    ...
    Some("wss") => {
      let tls_config = create_client_config(root_cert_store, vec![], unsafely_ignore_certificate_errors, None)?;
      let tls_connector = TlsConnector::from(Arc::new(tls_config));
      let dnsname = ServerName::try_from(domain.as_str()).map_err(|_| invalid_hostname(domain))?;
...
impl ServerWebSocket {
  #[inline]
  pub async fn write_frame(self: Rc<Self>, frame: Frame) -> Result<(), AnyError> {
    ...
    ws.write_frame(frame).await.map_err(|err| type_error(err.to_string()))?;
    Ok(())
  }
}
...
pub fn ws_create_server_stream(state: &mut OpState, transport: NetworkStream, read_buf: Bytes) -> Result<ResourceId, AnyError> {
  let mut ws = WebSocket::after_handshake(
    WebSocketStream::new(stream::WsStreamKind::Network(transport), Some(read_buf)),
    Role::Server,
  );
  ws.set_writev(true);
...
#[op]
pub async fn op_ws_send_binary(state: Rc<RefCell<OpState>>, rid: ResourceId, data: ZeroCopyBuf) -> Result<(), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
  resource.write_frame(Frame::new(true, OpCode::Binary, None, data.to_vec())).await
}

#[op]
pub async fn op_ws_send_text(state: Rc<RefCell<OpState>>, rid: ResourceId, data: String) -> Result<(), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
  resource.write_frame(Frame::new(true, OpCode::Text, None, data.into_bytes())).await
}

#[op]
pub async fn op_ws_send_pong(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
  resource.write_frame(Frame::pong(vec![])).await
}

#[op]
pub async fn op_ws_send_ping(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
  resource.write_frame(Frame::new(true, OpCode::Ping, None, vec![])).await
}

#[op(deferred)]
pub async fn op_ws_close(state: Rc<RefCell<OpState>>, rid: ResourceId, code: Option<u16>, reason: Option<String>) -> Result<(), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
  let frame = reason
...
#[op(fast)]
pub async fn op_ws_next_event(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(u16, StringOrBuffer), AnyError> {
  let resource = state.borrow_mut().resource_table.get::<ServerWebSocket>(rid)?;
    ...
    let val = match ws.read_frame().await {
      ...
      Err(err) => {
        ...
        if resource.closed.get() {
          ...
          return Ok((MessageKind::Closed as u16, StringOrBuffer::Buffer(vec![].into())));
        }

        return Ok((MessageKind::Error as u16, StringOrBuffer::String(err.to_string())));
      }
    };

    break Ok(match val.opcode {
      OpCode::Text => (MessageKind::Text as u16, StringOrBuffer::String(String::from_utf8(val.payload).unwrap())),
      OpCode::Binary => (MessageKind::Binary as u16, StringOrBuffer::Buffer(val.payload.into())),
      OpCode::Close => {
        if val.payload.len() < 2 {
          return Ok((1005, StringOrBuffer::String("".to_string())));
        }

        let close_code = CloseCode::from(u16::from_be_bytes([val.payload[0], val.payload[1]]));
        let reason = String::from_utf8(val.payload[2..].to_vec()).unwrap();
        (close_code.into(), StringOrBuffer::String(reason))
      }
      OpCode::Pong => (MessageKind::Pong as u16, StringOrBuffer::Buffer(vec![].into())),
...
impl DomExceptionNetworkError {
  pub fn new(msg: &str) -> Self {
    DomExceptionNetworkError { msg: msg.to_string() }
  }
}
...
pub fn get_network_error_class_name(e: &AnyError) -> Option<&'static str> {
  e.downcast_ref::<DomExceptionNetworkError>().map(|_| "DOMExceptionNetworkError")
}
