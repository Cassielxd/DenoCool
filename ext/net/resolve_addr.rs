...
pub async fn resolve_addr(hostname: &str, port: u16) -> Result<impl Iterator<Item = SocketAddr> + '_, AnyError> {
...
pub fn resolve_addr_sync(hostname: &str, port: u16) -> Result<impl Iterator<Item = SocketAddr>, AnyError> {
  let addr_port_pair = make_addr_port_pair(hostname, port);
...
mod tests {
  ...
  async fn resolve_addr1() {
    let expected = vec![SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(127, 0, 0, 1), 80))];
    let actual = resolve_addr("127.0.0.1", 80).await.unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }

  #[tokio::test]
  async fn resolve_addr2() {
    let expected = vec![SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 80))];
  ...
  async fn resolve_addr3() {
    let expected = vec![SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(192, 0, 2, 1), 25))];
    let actual = resolve_addr("192.0.2.1", 25).await.unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }
  ...
  async fn resolve_addr_ipv6() {
    ...
    let actual = resolve_addr("[2001:db8::1]", 8080).await.unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }
  ...
  fn resolve_addr_sync1() {
    let expected = vec![SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(127, 0, 0, 1), 80))];
    let actual = resolve_addr_sync("127.0.0.1", 80).unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }
  ...
  fn resolve_addr_sync3() {
    let expected = vec![SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(192, 0, 2, 1), 25))];
    let actual = resolve_addr_sync("192.0.2.1", 25).unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }
  ...
  fn resolve_addr_sync_ipv6() {
    ...
    let actual = resolve_addr_sync("[2001:db8::1]", 8080).unwrap().collect::<Vec<_>>();
    assert_eq!(actual, expected);
  }
