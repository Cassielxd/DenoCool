...
/// recv 一个TcpStream 创建TcpStreamResource
/// 这里本来是要从 resource_table获取 TcpListener 在这里被系统接管了
#[op]
async fn op_net_accept_tcp(state: Rc<RefCell<OpState>>, _rid: ResourceId) -> Result<(ResourceId, IpAddr, IpAddr), AnyError> {
  let acceptors_rx = {
    //这里不能直接take 否则下次来的时候没办法再次获取 已经被回收
    let mut op_state = state.borrow_mut();
    op_state.borrow_mut::<async_channel::Receiver<TcpStream>>().clone()
  };
  select! {
    Ok(tcp_stream) =  acceptors_rx.recv()=>{
     let local_addr = tcp_stream.local_addr()?;
     let remote_addr = tcp_stream.peer_addr()?;
     let mut state = state.borrow_mut();
     let rid = state.resource_table.add(TcpStreamResource::new(tcp_stream.into_split()));
...
#[op]
async fn op_net_recv_udp(state: Rc<RefCell<OpState>>, rid: ResourceId, mut buf: ZeroCopyBuf) -> Result<(usize, IpAddr), AnyError> {
  let resource = state
    .borrow_mut()
    .resource_table
  ...
  let (nread, remote_addr) = socket.recv_from(&mut buf).try_or_cancel(cancel_handle).await?;
  Ok((nread, IpAddr::from(remote_addr)))
}

#[op]
async fn op_net_send_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, addr: IpAddr, zero_copy: ZeroCopyBuf) -> Result<usize, AnyError>
where
...
{
  {
    ...
    s.borrow_mut::<NP>()
      .check_net(&(&addr.hostname, Some(addr.port)), "Deno.DatagramConn.send()")?;
  }
...
#[op]
async fn op_net_join_multi_v4_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, address: String, multi_interface: String) -> Result<(), AnyError>
where
  NP: NetPermissions + 'static,
{
  let resource = state
...
#[op]
async fn op_net_join_multi_v6_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, address: String, multi_interface: u32) -> Result<(), AnyError>
where
  NP: NetPermissions + 'static,
{
  let resource = state
...
#[op]
async fn op_net_leave_multi_v4_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, address: String, multi_interface: String) -> Result<(), AnyError>
where
  NP: NetPermissions + 'static,
{
  let resource = state
...
#[op]
async fn op_net_leave_multi_v6_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, address: String, multi_interface: u32) -> Result<(), AnyError>
where
  NP: NetPermissions + 'static,
{
  let resource = state
...
#[op]
async fn op_net_set_multi_ttl_udp<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, ttl: u32) -> Result<(), AnyError>
where
  NP: NetPermissions + 'static,
{
  let resource = state
...
#[op]
pub async fn op_net_connect_tcp<NP>(state: Rc<RefCell<OpState>>, addr: IpAddr) -> Result<(ResourceId, IpAddr, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
  {
  ...
  let mut state_ = state.borrow_mut();
  let rid = state_.resource_table.add(TcpStreamResource::new(tcp_stream.into_split()));
...
///此方法不做任何处理 接管原有的 op_net_listen_tcp方法
///直接返回ResourceId 和IpAddr
/// 如果想查看原方法实现 请查看deno 1.33源代码 op_net_listen_tcp方法的实现
#[op]
fn op_net_listen_tcp<NP>(_state: &mut OpState, addr: IpAddr, _reuse_port: bool) -> Result<(ResourceId, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
  Ok((1, addr))
...
fn net_listen_udp<NP>(state: &mut OpState, addr: IpAddr, reuse_address: bool, loopback: bool) -> Result<(ResourceId, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
  state
...
#[op]
fn op_net_listen_udp<NP>(state: &mut OpState, addr: IpAddr, reuse_address: bool, loopback: bool) -> Result<(ResourceId, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
...
#[op]
pub async fn op_dns_resolve<NP>(state: Rc<RefCell<OpState>>, args: ResolveAddrArgs) -> Result<Vec<DnsReturnRecord>, AnyError>
where
  NP: NetPermissions + 'static,
{
  let ResolveAddrArgs {
  ...
  let (config, opts) = if let Some(name_server) = options.as_ref().and_then(|o| o.name_server.as_ref()) {
    let group = NameServerConfigGroup::from_ips_clear(&[name_server.ip_addr.parse()?], name_server.port, true);
    (ResolverConfig::from_parts(None, vec![], group), ResolverOpts::default())
  } else {
  ...
  let cancel_handle = cancel_rid.and_then(|rid| state.borrow_mut().resource_table.get::<CancelHandle>(rid).ok());
  ...
  lookup
    .map_err(|e| {
      ...
      match e.kind() {
        ResolveErrorKind::NoRecordsFound { .. } => custom_error("NotFound", message),
        ResolveErrorKind::Message("No connections available") => custom_error("NotConnected", message),
...
#[op]
pub fn op_set_nodelay(state: &mut OpState, rid: ResourceId, nodelay: bool) -> Result<(), AnyError> {
  let resource: Rc<TcpStreamResource> = state.resource_table.get::<TcpStreamResource>(rid)?;
  resource.set_nodelay(nodelay)
}

#[op]
pub fn op_set_keepalive(state: &mut OpState, rid: ResourceId, keepalive: bool) -> Result<(), AnyError> {
  let resource: Rc<TcpStreamResource> = state.resource_table.get::<TcpStreamResource>(rid)?;
  resource.set_keepalive(keepalive)
}

fn rdata_to_return_record(ty: RecordType) -> impl Fn(&RData) -> Result<Option<DnsReturnRecord>, AnyError> {
  use RecordType::*;
  move |r: &RData| -> Result<Option<DnsReturnRecord>, AnyError> {
    let record = match ty {
      A => r.as_a().map(ToString::to_string).map(DnsReturnRecord::A),
      AAAA => r.as_aaaa().map(ToString::to_string).map(DnsReturnRecord::Aaaa),
      ANAME => r.as_aname().map(ToString::to_string).map(DnsReturnRecord::Aname),
      ...
      CNAME => r.as_cname().map(ToString::to_string).map(DnsReturnRecord::Cname),
      ...
      PTR => r.as_ptr().map(ToString::to_string).map(DnsReturnRecord::Ptr),
...
mod tests {
  ...
  fn rdata_to_return_record_a() {
    ...
    assert_eq!(func(&rdata).unwrap(), Some(DnsReturnRecord::A("127.0.0.1".to_string())));
  }
  ...
  fn rdata_to_return_record_soa() {
    ...
    let rdata = RData::SOA(SOA::new(Name::new(), Name::new(), 0, i32::MAX, i32::MAX, i32::MAX, 0));
    assert_eq!(
      func(&rdata).unwrap(),
  ...
  async fn check_sockopt(addr: String, set_sockopt_fn: Box<dyn Fn(&mut OpState, u32)>, test_fn: Box<dyn FnOnce(SockRef)>) {
    ...
    let mut connect_fut = op_net_connect_tcp::call::<TestPermission>(conn_state, ip_addr).boxed_local();
    let mut rid = None;
    ...
    let resource = state.borrow_mut().resource_table.get::<TcpStreamResource>(rid).unwrap();
