...
pub type TcpStreamResource = FullDuplexResource<tcp::OwnedReadHalf, tcp::OwnedWriteHalf>;
...
impl TcpStreamResource {
  ...
  pub fn set_keepalive(self: Rc<Self>, keepalive: bool) -> Result<(), AnyError> {
    self.map_socket(Box::new(move |socket| Ok(socket.set_keepalive(keepalive)?)))
  }

  #[allow(clippy::type_complexity)]
  fn map_socket(self: Rc<Self>, map: Box<dyn FnOnce(SockRef) -> Result<(), AnyError>>) -> Result<(), AnyError> {
...
#[cfg(unix)]
pub type UnixStreamResource = FullDuplexResource<unix::OwnedReadHalf, unix::OwnedWriteHalf>;
