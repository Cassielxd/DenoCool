...
impl TlsStream {
  ...
  pub fn new_client_side(tcp: TcpStream, tls_config: Arc<ClientConfig>, server_name: ServerName) -> Self {
  ...
  pub fn into_split(self) -> (ReadHalf, WriteHalf) {
    ...
    let rd = ReadHalf { shared: shared.clone() };
...
impl TlsStreamInner {
  fn poll_io(&mut self, cx: &mut Context<'_>, flow: Flow) -> Poll<io::Result<()>> {
    loop {
      let wr_ready = loop {
        match self.wr_state {
      ...
      let rd_ready = loop {
        ...
        let tls_state = self.tls.process_new_packets().map_err(|e| Error::new(ErrorKind::InvalidData, e))?;
      ...
      if wr_ready {
        if self.rd_state >= State::TlsClosed && self.wr_state >= State::TlsClosed && self.wr_state < State::TcpClosed {
          continue;
        }
...
impl AsyncRead for ReadHalf {
  fn poll_read(self: Pin<&mut Self>, cx: &mut Context<'_>, buf: &mut ReadBuf<'_>) -> Poll<io::Result<()>> {
    self.shared.poll_with_shared_waker(cx, Flow::Read, move |tls, cx| tls.poll_read(cx, buf))
  }
}
...
impl WriteHalf {
  pub async fn handshake(&mut self) -> io::Result<()> {
    poll_fn(|cx| self.shared.poll_with_shared_waker(cx, Flow::Write, |mut tls, cx| tls.poll_handshake(cx))).await
  }
...
impl AsyncWrite for WriteHalf {
  ...
  fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
    self.shared.poll_with_shared_waker(cx, Flow::Write, |tls, cx| tls.poll_flush(cx))
  }

  fn poll_shutdown(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
    self.shared.poll_with_shared_waker(cx, Flow::Write, |tls, cx| tls.poll_shutdown(cx))
  }
}
...
#[op]
pub async fn op_tls_start<NP>(state: Rc<RefCell<OpState>>, args: StartTlsArgs) -> Result<(ResourceId, IpAddr, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
  ...
  let ca_certs = args.ca_certs.into_iter().map(|s| s.into_bytes()).collect::<Vec<_>>();

  let hostname_dns = ServerName::try_from(hostname).map_err(|_| invalid_hostname(hostname))?;

  let unsafely_ignore_certificate_errors = state.borrow().try_borrow::<UnsafelyIgnoreCertificateErrors>().and_then(|it| it.0.clone());

  let root_cert_store = state.borrow().borrow::<DefaultTlsOptions>().root_cert_store()?;

  let resource_rc = state.borrow_mut().resource_table.take::<TcpStreamResource>(rid)?;
  ...
  let resource = Rc::try_unwrap(resource_rc).map_err(|_| bad_resource("TCP stream is currently in use"))?;
  ...
  let mut tls_config = create_client_config(root_cert_store, ca_certs, unsafely_ignore_certificate_errors, None)?;
  ...
  let tls_stream = TlsStream::new_client_side(tcp_stream, tls_config, hostname_dns);

  let rid = {
    let mut state_ = state.borrow_mut();
    state_.resource_table.add(TlsStreamResource::new(tls_stream.into_split()))
  };
...
#[op]
pub async fn op_net_connect_tls<NP>(state: Rc<RefCell<OpState>>, addr: IpAddr, args: ConnectTlsArgs) -> Result<(ResourceId, IpAddr, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
...

  {
    let mut s = state.borrow_mut();
    let permissions = s.borrow_mut::<NP>();
    permissions.check_net(&(&addr.hostname, Some(addr.port)), "Deno.connectTls()")?;
    if let Some(path) = cert_file {
      permissions.check_read(Path::new(path), "Deno.connectTls()")?;
    }
  }

  let mut ca_certs = args.ca_certs.into_iter().map(|s| s.into_bytes()).collect::<Vec<_>>();
  ...
  let root_cert_store = state.borrow().borrow::<DefaultTlsOptions>().root_cert_store()?;
  let hostname_dns = ServerName::try_from(&*addr.hostname).map_err(|_| invalid_hostname(&addr.hostname))?;
  ...
  let cert_chain_and_key = if args.cert_chain.is_some() || args.private_key.is_some() {
  ...
  let mut tls_config = create_client_config(root_cert_store, ca_certs, unsafely_ignore_certificate_errors, cert_chain_and_key)?;
...
#[op]
pub fn op_net_listen_tls<NP>(state: &mut OpState, addr: IpAddr, args: ListenTlsArgs) -> Result<(ResourceId, IpAddr), AnyError>
where
  NP: NetPermissions + 'static,
{
  if args.reuse_port {
...

  {
    ...
    permissions.check_net(&(&addr.hostname, Some(addr.port)), "Deno.listenTls()")?;
    if let Some(path) = cert_file {
  ...
  let key_der = if key_file.is_some() && key.is_some() {
    return Err(generic_error("Both key and keyFile is specified. You can specify either one of them."));
...
#[op]
pub async fn op_net_accept_tls(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(ResourceId, IpAddr, IpAddr), AnyError> {
  let resource = state
    .borrow()
    .resource_table
  ...
  let (tcp_stream, remote_addr) = match tcp_listener.accept().try_or_cancel(&cancel_handle).await {
    Ok(tuple) => tuple,
  ...
  let tls_stream = TlsStream::new_server_side(tcp_stream, resource.tls_config.clone());
...
#[op]
pub async fn op_tls_handshake(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<TlsHandshakeInfo, AnyError> {
  let resource = state.borrow().resource_table.get::<TlsStreamResource>(rid)?;
