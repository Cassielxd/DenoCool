...
#[op]
pub async fn op_net_accept_unix(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(ResourceId, Option<String>, Option<String>), AnyError> {
  let resource = state
    .borrow()
    .resource_table
  ...
  let (unix_stream, _socket_addr) = listener.accept().try_or_cancel(cancel).await.map_err(crate::ops::accept_err)?;

  let local_addr = unix_stream.local_addr()?;
  let remote_addr = unix_stream.peer_addr()?;
  let local_addr_path = local_addr.as_pathname().map(pathstring).transpose()?;
  let remote_addr_path = remote_addr.as_pathname().map(pathstring).transpose()?;
...
#[op]
pub async fn op_net_connect_unix<NP>(state: Rc<RefCell<OpState>>, path: String) -> Result<(ResourceId, Option<String>, Option<String>), AnyError>
where
  NP: NetPermissions + 'static,
{
  ...
  super::check_unstable2(&state, "Deno.connect");
  {
    ...
    state_.borrow_mut::<NP>().check_read(address_path, "Deno.connect()")?;
    state_.borrow_mut::<NP>().check_write(address_path, "Deno.connect()")?;
  }
...
#[op]
pub async fn op_net_recv_unixpacket(state: Rc<RefCell<OpState>>, rid: ResourceId, mut buf: ZeroCopyBuf) -> Result<(usize, Option<String>), AnyError> {
  let resource = state
    .borrow()
    .resource_table
  ...
  let cancel = RcRef::map(resource, |r| &r.cancel);
  let (nread, remote_addr) = socket.recv_from(&mut buf).try_or_cancel(cancel).await?;
  let path = remote_addr.as_pathname().map(pathstring).transpose()?;
  Ok((nread, path))
}

#[op]
async fn op_net_send_unixpacket<NP>(state: Rc<RefCell<OpState>>, rid: ResourceId, path: String, zero_copy: ZeroCopyBuf) -> Result<usize, AnyError>
where
  NP: NetPermissions + 'static,
{
  let address_path = Path::new(&path);
  {
    ...
    s.borrow_mut::<NP>().check_write(address_path, "Deno.DatagramConn.send()")?;
  }
...
#[op]
pub fn op_net_listen_unix<NP>(state: &mut OpState, path: String) -> Result<(ResourceId, Option<String>), AnyError>
where
  NP: NetPermissions + 'static,
{
  let address_path = Path::new(&path);
...
pub fn net_listen_unixpacket<NP>(state: &mut OpState, path: String) -> Result<(ResourceId, Option<String>), AnyError>
where
  NP: NetPermissions + 'static,
{
  let address_path = Path::new(&path);
...
#[op]
pub fn op_net_listen_unixpacket<NP>(state: &mut OpState, path: String) -> Result<(ResourceId, Option<String>), AnyError>
where
  NP: NetPermissions + 'static,
{
...
#[op]
pub fn op_node_unstable_net_listen_unixpacket<NP>(state: &mut OpState, path: String) -> Result<(ResourceId, Option<String>), AnyError>
where
  NP: NetPermissions + 'static,
{
  net_listen_unixpacket::<NP>(state, path)
