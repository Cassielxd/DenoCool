...
trait PollFrame: Unpin {
  fn poll_frame(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<ResponseStreamResult>;
...
#[derive(Debug, Default)]
pub struct ResponseBytes(ResponseBytesInner, CompletionHandle, Rc<RefCell<Option<HeaderMap>>>);
...
impl ResponseBytesInner {
  ...
  fn from_stream(compression: Compression, stream: ResponseStream) -> Self {
    match compression {
      ...
      Compression::Brotli => Self::BrotliStream(BrotliResponseStream::new(stream)),
  ...
  pub fn from_resource(compression: Compression, stm: Rc<dyn Resource>, auto_close: bool) -> Self {
    Self::from_stream(compression, ResponseStream::Resource(ResourceBodyAdapter::new(stm, auto_close)))
  }
  ...
  pub fn from_slice(compression: Compression, bytes: &[u8]) -> Self {
    match compression {
      ...
      Compression::Brotli => {
        ...
        // (~4MB)
        let mut writer = brotli::CompressorWriter::new(Vec::new(), 65 * 1024, 6, 22);
        writer.write_all(bytes).unwrap();
        writer.flush().unwrap();
...
impl Body for ResponseBytes {
  ...
  fn poll_frame(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Option<Result<Frame<Self::Data>, Self::Error>>> {
    let res = loop {
  ...
  fn is_end_stream(&self) -> bool {
    matches!(self.0, ResponseBytesInner::Done | ResponseBytesInner::Empty) && self.2.borrow_mut().is_none()
  }
...
impl ResourceBodyAdapter {
  pub fn new(stm: Rc<dyn Resource>, auto_close: bool) -> Self {
    ...
    ResourceBodyAdapter { auto_close, stm, future }
  }
}

impl PollFrame for ResponseStream {
  fn poll_frame(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<ResponseStreamResult> {
    match &mut *self {
...
static GZIP_HEADER: Bytes = Bytes::from_static(&[0x1f, 0x8b, 0x08, 0, 0, 0, 0, 0, 0x01, 0xff]);
...
impl PollFrame for GZipResponseStream {
  fn poll_frame(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<ResponseStreamResult> {
    ...
    let frame = match *state {
      GZipState::EndOfStream => return std::task::Poll::Ready(ResponseStreamResult::EndOfStream),
      GZipState::Header => {
        *state = GZipState::Streaming;
        return std::task::Poll::Ready(ResponseStreamResult::NonEmptyBuf(BufView::from(GZIP_HEADER.clone())));
      }
      GZipState::Trailer => {
        *state = GZipState::EndOfStream;
        ...
        return std::task::Poll::Ready(ResponseStreamResult::NonEmptyBuf(BufView::from(v)));
      }
      GZipState::Streaming => {
    ...
    let mut buf = this.next_buf.take().unwrap_or_else(|| BytesMut::zeroed(64 * 1024));

    let start_in = stm.total_in();
    let start_out = stm.total_out();
    let res = match frame {
      // Short-circuit these and just return
      x @ (ResponseStreamResult::NoData | ResponseStreamResult::Error(..) | ResponseStreamResult::Trailers(..)) => return std::task::Poll::Ready(x),
      ResponseStreamResult::EndOfStream => {
  ...
  fn poll_frame(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<ResponseStreamResult> {
    ...
    let res = match frame {
      ResponseStreamResult::NonEmptyBuf(buf) => {
        ...
        output_buffer.truncate(total_output_written - this.output_written_so_far);
...
pub struct V8StreamHttpResponseBody(AsyncRefCell<Option<tokio::sync::mpsc::Sender<BufView>>>, CancelHandle);
    ...
    Box::pin(
      async move {
        ...
        if let Some(tx) = res.as_ref() {
          tx.send(buf).await.map_err(|_| bad_resource("failed to write"))?;
...
mod tests {
  ...
  fn already_gzipped_data() -> Vec<u8> {
    ...
    let mut gz = flate2::GzBuilder::new().write(&mut v, flate2::Compression::best());
