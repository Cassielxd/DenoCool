    ...
    loop {
      ...
      match state {
        ...
        State::Matched => {
          ...
          return Ok((true, NetworkBufferedStream::new(io, buffer, initialized_len)));
        }
        State::NotMatched => {
          let initialized_len = readbuf.filled().len();
          return Ok((false, NetworkBufferedStream::new(io, buffer, initialized_len)));
        }
      }
    }
  }
...
impl<S: AsyncRead + Unpin> NetworkBufferedStream<S> {
  ...
  fn new(io: S, prefix: [MaybeUninit<u8>; MAX_PREFIX_SIZE * 2], initialized_len: usize) -> Self {
    Self {
      io,
      initialized_len,
      prefix_offset: 0,
...
impl<S: AsyncRead + AsyncWrite + Unpin> AsyncWrite for NetworkBufferedStream<S> {
  fn poll_write(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>, buf: &[u8]) -> std::task::Poll<Result<usize, std::io::Error>> {
...
mod tests {
  ...
  impl AsyncRead for YieldsOneByteAtATime {
    fn poll_read(mut self: Pin<&mut Self>, _cx: &mut std::task::Context<'_>, buf: &mut ReadBuf<'_>) -> Poll<io::Result<()>> {
  ...
  async fn test(io: impl AsyncRead + Unpin, prefix: &'static [u8], expect_match: bool, expect_string: &'static str) -> io::Result<()> {
    let (matches, mut io) = NetworkStreamPrefixCheck::new(io, prefix).match_prefix().await?;
