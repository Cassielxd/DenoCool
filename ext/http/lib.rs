...
impl HttpConnResource {
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
  {
    ...
    let conn_fut = Http::new().with_executor(LocalExecutor).serve_connection(io, service).with_upgrades();
  ...
  async fn accept(self: &Rc<Self>) -> Result<Option<(HttpStreamResource, String, String)>, AnyError> {
    let fut = async {
      ...
      let accept_encoding = {
        let encodings =
          fly_accept_encoding::encodings_iter(request.headers()).filter(|r| matches!(r, Ok((Some(Encoding::Brotli | Encoding::Gzip), _))));
      ...
      let stream = HttpStreamResource::new(self, request, response_tx, accept_encoding);
...
pub fn http_create_conn_resource<S, A>(state: &mut OpState, io: S, addr: A, scheme: &'static str) -> Result<ResourceId, AnyError>
where
  S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
...
impl Service<Request<Body>> for HttpService {
  ...
  fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
    ...
    let result = ready!(acceptors_rx.poll_peek(cx)).map(|_| ()).ok_or(oneshot::Canceled);
    Poll::Ready(result)
  }
...
impl HttpAcceptor {
  fn new(request_tx: oneshot::Sender<Request<Body>>, response_rx: oneshot::Receiver<Response<Body>>) -> Self {
    Self { request_tx, response_rx }
  }

  fn call(self, request: Request<Body>) -> oneshot::Receiver<Response<Body>> {
    let Self { request_tx, response_rx } = self;
    request_tx.send(request).map(|_| response_rx).unwrap_or_else(|_| oneshot::channel().1)
...
impl HttpStreamResource {
  fn new(conn: &Rc<HttpConnResource>, request: Request<Body>, response_tx: oneshot::Sender<Response<Body>>, accept_encoding: Encoding) -> Self {
...
#[op]
async fn op_http_accept(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<Option<NextRequestResponse>, AnyError> {
  let conn = state.borrow().resource_table.get::<HttpConnResource>(rid)?;

  match conn.accept().await {
    Ok(Some((stream, method, url))) => {
      let stream_rid = state.borrow_mut().resource_table.add_rc(Rc::new(stream));
...
fn req_url(req: &hyper::Request<hyper::Body>, scheme: &'static str, addr: &HttpSocketAddr) -> String {
  let host: Cow<str> = match addr {
    HttpSocketAddr::IpSocket(addr) => {
      if let Some(auth) = req.uri().authority() {
        ...
        match host.to_str() {
          ...
          Err(_) => Cow::Owned(host.as_bytes().iter().cloned().map(char::from).collect::<String>()),
        }
      } else {
    ...
    HttpSocketAddr::UnixSocket(addr) => Cow::Owned(
      percent_encoding::percent_encode(
        addr.as_pathname().and_then(|x| x.to_str()).unwrap_or_default().as_bytes(),
...
#[op]
fn op_http_headers(state: &mut OpState, rid: u32) -> Result<Vec<(ByteString, ByteString)>, AnyError> {
...
#[op]
async fn op_http_write_resource(state: Rc<RefCell<OpState>>, rid: ResourceId, stream: ResourceId) -> Result<(), AnyError> {
...
#[op]
async fn op_http_write(state: Rc<RefCell<OpState>>, rid: ResourceId, buf: ZeroCopyBuf) -> Result<(), AnyError> {
...
#[op]
async fn op_http_shutdown(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<(), AnyError> {
  let stream = state.borrow().resource_table.get::<HttpStreamResource>(rid)?;
  let mut wr = RcRef::map(&stream, |r| &r.wr).borrow_mut().await;
  let wr = take(&mut *wr);
  match wr {
...
#[op]
async fn op_http_upgrade_websocket(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<ResourceId, AnyError> {
  let stream = state.borrow_mut().resource_table.get::<HttpStreamResource>(rid)?;
  ...
  let ws_rid = ws_create_server_stream(&mut state.borrow_mut(), transport, bytes)?;
  Ok(ws_rid)
}
...
fn filter_enotconn(result: Result<(), hyper::Error>) -> Result<(), hyper::Error> {
  if result
    .as_ref()
    .err()
...
trait CanDowncastUpgrade: Sized {
  fn downcast<T: AsyncRead + AsyncWrite + Unpin + 'static>(self) -> Result<(T, Bytes), Self>;
}
...
fn maybe_extract_network_stream<T: Into<NetworkStream> + AsyncRead + AsyncWrite + Unpin + 'static, U: CanDowncastUpgrade>(
  upgraded: U,
...
fn extract_network_stream<U: CanDowncastUpgrade>(upgraded: U) -> (NetworkStream, Bytes) {
  let upgraded = match maybe_extract_network_stream::<tokio::net::TcpStream, _>(upgraded) {
    Ok(res) => return res,
    Err(x) => x,
  };
  let upgraded = match maybe_extract_network_stream::<deno_net::ops_tls::TlsStream, _>(upgraded) {
    Ok(res) => return res,
    Err(x) => x,
  };
  #[cfg(unix)]
  let upgraded = match maybe_extract_network_stream::<tokio::net::UnixStream, _>(upgraded) {
    Ok(res) => return res,
    Err(x) => x,
  };
  let upgraded = match maybe_extract_network_stream::<NetworkStream, _>(upgraded) {
    Ok(res) => return res,
    Err(x) => x,
  };
