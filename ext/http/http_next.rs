...
/// client and service each send a different connection preface.
///
...
trait HttpServeStream: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static {}
impl<S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static> HttpServeStream for S {}

#[op]
pub fn op_http_upgrade_raw(state: &mut OpState, slab_id: SlabId) -> Result<ResourceId, AnyError> {
  ...
  Ok(state.resource_table.add(UpgradeStream::new(read_rx, write_tx)))
}
...
pub async fn op_http_upgrade_websocket_next(
  ...
  *response.status_mut() = StatusCode::SWITCHING_PROTOCOLS;
  for (name, value) in headers {
    response
      .headers_mut()
      .append(HeaderName::from_bytes(&name).unwrap(), HeaderValue::from_bytes(&value).unwrap());
  }
  http.complete();
...
#[op]
pub fn op_http_get_request_method_and_url<HTTP>(slab_id: SlabId) -> (String, Option<String>, String, String, Option<u16>)
where
  HTTP: HttpPropertyExtractor,
{
  ...
  let request_properties = HTTP::request_properties(request_info, &request_parts.uri, &request_parts.headers);
...
#[op]
pub fn op_http_get_request_header(slab_id: SlabId, name: String) -> Option<ByteString> {
  let http = slab_get(slab_id);
...
#[op]
pub fn op_http_get_request_headers(slab_id: SlabId) -> Vec<(ByteString, ByteString)> {
  let http = slab_get(slab_id);
  ...
  if let Some(cookies) = cookies {
    ...
    vec.push((ByteString::from(COOKIE.as_str()), ByteString::from(cookies.join(cookie_sep))));
  }
  vec
}

#[op(fast)]
pub fn op_http_read_request_body(state: &mut OpState, slab_id: SlabId) -> ResourceId {
  let mut http = slab_get(slab_id);
...
#[op]
pub fn op_http_set_response_headers(slab_id: SlabId, headers: Vec<(ByteString, ByteString)>) {
  let mut http = slab_get(slab_id);
...
#[op]
pub fn op_http_set_response_trailers(slab_id: SlabId, trailers: Vec<(ByteString, ByteString)>) {
  let mut http = slab_get(slab_id);
...
fn is_request_compressible(headers: &HeaderMap) -> Compression {
  ...
  let accepted =
    fly_accept_encoding::encodings_iter(headers).filter(|r| matches!(r, Ok((Some(Encoding::Identity | Encoding::Gzip | Encoding::Brotli), _))));
...
fn modify_compressibility_from_response(compression: Compression, length: Option<usize>, headers: &mut HeaderMap) -> Compression {
...
fn set_response(slab_id: SlabId, length: Option<usize>, response_fn: impl FnOnce(Compression) -> ResponseBytesInner) {
  let mut http = slab_get(slab_id);
  let compression = is_request_compressible(&http.request_parts().headers);
  let response = http.response();
  let compression = modify_compressibility_from_response(compression, length, response.headers_mut());
  response.body_mut().initialize(response_fn(compression))
}

#[op(fast)]
pub fn op_http_set_response_body_resource(state: &mut OpState, slab_id: SlabId, stream_rid: ResourceId, auto_close: bool) -> Result<(), AnyError> {
  ...
  set_response(slab_id, resource.size_hint().1.map(|s| s as usize), move |compression| {
...
#[op(fast)]
pub fn op_http_set_response_body_stream(state: &mut OpState, slab_id: SlabId) -> Result<ResourceId, AnyError> {
  // TODO(mmastrac): what should this channel size be?
  let (tx, rx) = tokio::sync::mpsc::channel(1);
  set_response(slab_id, None, |compression| ResponseBytesInner::from_v8(compression, rx));
...
#[op]
pub async fn op_http_track(state: Rc<RefCell<OpState>>, slab_id: SlabId, server_rid: ResourceId) -> Result<(), AnyError> {
  let http = slab_get(slab_id);
  let handle = http.body_promise();

  let join_handle = state.borrow_mut().resource_table.get::<HttpJoinHandle>(server_rid)?;
...
impl<F: Future<Output = ()>> Future for SlabFuture<F> {
  ...
  fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Self::Output> {
    let index = self.0;
    self.project().1.poll(cx).map(|_| Ok(slab_get(index).take_response()))
  }
}
...
fn serve_http11_unconditional(
  ...
  let conn = http1::Builder::new().keep_alive(true).serve_connection(io, svc);
...
fn serve_https(
  ...
  let svc = service_fn(move |req: Request| new_slab_future(req, request_info.clone(), tx.clone()));
  spawn(
    async {
      io.handshake().await?;
...
where
  HTTP: HttpPropertyExtractor,
{
  let connection_properties: HttpConnectionProperties = HTTP::connection_properties(listen_properties, &connection);

  let network_stream = HTTP::to_network_stream_from_connection(connection);

  match network_stream {
    NetworkStream::Tcp(conn) => serve_http(conn, connection_properties, cancel, tx),
    NetworkStream::Tls(conn) => serve_https(conn, connection_properties, cancel, tx),
    #[cfg(unix)]
    NetworkStream::Unix(conn) => serve_http(conn, connection_properties, cancel, tx),
  }
}
...
#[op(v8)]
pub fn op_http_serve<HTTP>(state: Rc<RefCell<OpState>>, listener_rid: ResourceId) -> Result<(ResourceId, &'static str, String), AnyError>
where
  HTTP: HttpPropertyExtractor,
{
  let listener = HTTP::get_listener_for_rid(&mut state.borrow_mut(), listener_rid)?;

  let listen_properties = HTTP::listen_properties_from_listener(&listener)?;

  let (tx, rx) = tokio::sync::mpsc::channel(10);
  let resource: Rc<HttpJoinHandle> = Rc::new(HttpJoinHandle(AsyncRefCell::new(None), CancelHandle::new_rc(), AsyncRefCell::new(rx)));
  ...
  let handle = spawn(async move {
    loop {
      let conn = HTTP::accept_connection_from_listener(&listener)
        ...
        .await?;
      serve_http_on::<HTTP>(conn, &listen_properties_clone, cancel_clone.clone(), tx.clone());
    }
  ...
  *RcRef::map(&resource, |this| &this.0).try_borrow_mut().unwrap() = Some(handle);
...
#[op(v8)]
pub fn op_http_serve_on<HTTP>(state: Rc<RefCell<OpState>>, connection_rid: ResourceId) -> Result<(ResourceId, &'static str, String), AnyError>
where
  HTTP: HttpPropertyExtractor,
{
  let connection = HTTP::get_connection_for_rid(&mut state.borrow_mut(), connection_rid)?;
  ...
  let handle: JoinHandle<Result<(), deno_core::anyhow::Error>> = serve_http_on::<HTTP>(connection, &listen_properties, resource.cancel_handle(), tx);
...
#[op]
pub async fn op_http_wait(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<SlabId, AnyError> {
  // We will get the join handle initially, as we might be consuming requests still
  let join_handle = state.borrow_mut().resource_table.get::<HttpJoinHandle>(rid)?;
  ...
  let res = RcRef::map(join_handle, |this| &this.0).borrow_mut().await.take().unwrap().await?;
...
impl UpgradeStream {
  pub fn new(read: tokio::io::ReadHalf<tokio::io::DuplexStream>, write: tokio::io::WriteHalf<tokio::io::DuplexStream>) -> Self {
    Self {
