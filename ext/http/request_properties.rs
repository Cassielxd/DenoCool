...
pub trait HttpPropertyExtractor {
  ...
  fn listen_properties_from_listener(listener: &Self::Listener) -> Result<HttpListenProperties, std::io::Error>;

  /// Determines the listener properties given a [`HttpPropertyExtractor::Connection`].
  fn listen_properties_from_connection(connection: &Self::Connection) -> Result<HttpListenProperties, std::io::Error>;

  /// Accept a new [`HttpPropertyExtractor::Connection`] from the given listener [`HttpPropertyExtractor::Listener`].
  async fn accept_connection_from_listener(listener: &Self::Listener) -> Result<Self::Connection, AnyError>;
...
impl HttpPropertyExtractor for DefaultHttpPropertyExtractor {
  ...
  fn get_listener_for_rid(state: &mut OpState, listener_rid: ResourceId) -> Result<NetworkStreamListener, AnyError> {
    take_network_stream_listener_resource(&mut state.resource_table, listener_rid)
  }
  ...
  async fn accept_connection_from_listener(listener: &NetworkStreamListener) -> Result<NetworkStream, AnyError> {
    listener.accept().await.map_err(Into::into)
  }

  fn listen_properties_from_listener(listener: &NetworkStreamListener) -> Result<HttpListenProperties, std::io::Error> {
  ...
  fn connection_properties(listen_properties: &HttpListenProperties, connection: &NetworkStream) -> HttpConnectionProperties {
    ...
    let peer_address = connection
      .peer_address()
      .unwrap_or_else(|_| NetworkStreamAddress::Ip(SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 0))));
...
fn listener_properties(stream_type: NetworkStreamType, local_address: NetworkStreamAddress) -> Result<HttpListenProperties, std::io::Error> {
...
fn req_host_from_addr(stream_type: NetworkStreamType, addr: &NetworkStreamAddress) -> String {
  match addr {
    NetworkStreamAddress::Ip(addr) => {
      if (stream_type == NetworkStreamType::Tls && addr.port() == 443) || (stream_type == NetworkStreamType::Tcp && addr.port() == 80) {
    ...
    NetworkStreamAddress::Unix(unix) => percent_encoding::percent_encode(
      unix.as_pathname().and_then(|x| x.to_str()).unwrap_or_default().as_bytes(),
...
fn req_host<'a>(uri: &'a Uri, headers: &'a HeaderMap, addr_type: NetworkStreamType, port: u16) -> Option<Cow<'a, str>> {
