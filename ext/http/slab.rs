...
pub struct SlabEntry(NonNull<HttpSlabRecord>, SlabId, RefMut<'static, Slab<HttpSlabRecord>>);
...
fn slab_insert_raw(request_parts: Parts, request_body: Option<Incoming>, request_info: HttpConnectionProperties) -> SlabId {
...
pub fn slab_insert(request: Request, request_info: HttpConnectionProperties) -> SlabId {
...
pub fn slab_drop(index: SlabId) {
  ...
  assert!(!record.been_dropped, "HTTP state error: Entry has already been dropped");
...
impl SlabEntry {
  ...
  pub fn complete(self) {
    ...
    assert!(!promise.is_completed(), "HTTP state error: Entry has already been completed");
  ...
  pub fn body_promise(&self) -> CompletionHandle {
    self.self_ref().response.as_ref().unwrap().body().completion_handle()
  }
}
