...
impl<T: Default> WebSocketUpgrade<T> {
  ...
  pub fn write(&mut self, bytes: &[u8]) -> Result<Option<(Response<T>, Bytes)>, AnyError> {
    ...
    match self.state {
      ...
      Headers => {
        self.buf.extend_from_slice(bytes);
        let header_searcher = HEADER_SEARCHER.get_or_init(|| TwoWaySearcher::new(b"\r\n\r\n"));
...
mod tests {
  ...
  fn assert_response(
    ...
    match expected {
      Ok(Some((resp1, remainder1))) => match result {
        Ok(Some((resp2, remainder2))) => {
          ...
          if let Some((byte_len, chunk_size)) = chunk_info {
            ...
            let last_packet_header_size = (byte_len - remainder1.len() + chunk_size - 1) % chunk_size + 1;

            // Which means we can compute how much was in the remainder
            let remaining = (chunk_size - last_packet_header_size).min(remainder1.len());
      ...
      Ok(None) => assert!(result.ok().unwrap().is_none(), "Expected Ok(None), was {formatted}",),
      Err(e) => assert_eq!(
        e,
  ...
  fn validate_upgrade_all_at_once(s: &str, expected: Result<ExpectedResponseAndHead, &'static str>) {
  ...
  fn validate_upgrade_chunks(s: &str, size: usize, expected: Result<ExpectedResponseAndHead, &'static str>) {
  ...
  fn validate_upgrade(s: &str, expected: fn() -> Result<ExpectedResponseAndHead, &'static str>) {
  ...
  #[test]
  fn upgrade1() {
    validate_upgrade("HTTP/1.1 101 Switching Protocols\nConnection: Upgrade\n\n", || {
      let mut expected = Response::builder().status(101).body(Body::empty()).unwrap();
      expected
        .headers_mut()
  ...
  fn upgrade2() {
    validate_upgrade("HTTP/1.1 101 Switching Protocols\nConnection: Upgrade\nOther: 123\n\n", || {
      ...
      expected
        .headers_mut()
        .append(HeaderName::from_static("other"), HeaderValue::from_static("123"));
      Ok(Some((expected, b"")))
    });
  }
  ...
  fn upgrade_too_many_headers() {
    let headers = (0..20).map(|i| format!("h{i}: {i}")).collect::<Vec<_>>().join("\n");
