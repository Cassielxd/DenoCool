...
impl FetchHandler for FsFetchHandler {
  fn fetch_file(&self, _state: &mut OpState, url: Url) -> (CancelableResponseFuture, Option<FetchRequestBodyResource>, Option<Rc<CancelHandle>>) {
    ...
    let response_fut = async move {
      ...
      let response = http::Response::builder().status(StatusCode::OK).body(body).map_err(|_| ())?.into();
      Ok::<_, ()>(response)
    }
    .map_err(move |_| type_error("NetworkError when attempting to fetch resource."))
