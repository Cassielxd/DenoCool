...
impl BootstrapOptions {
  pub fn as_v8<'s>(&self, scope: &mut v8::HandleScope<'s>) -> v8::Local<'s, v8::Array> {
    ...
    {
      let val = v8::String::new_from_one_byte(scope, self.runtime_version.as_bytes(), v8::NewStringType::Internalized).unwrap();
      array.set_index(scope, 3, val.into());
    }

    {
      let val = v8::String::new_from_one_byte(scope, self.locale.as_bytes(), v8::NewStringType::Normal).unwrap();
    ...
    {
      let val = v8::String::new_from_one_byte(scope, self.ts_version.as_bytes(), v8::NewStringType::Normal).unwrap();
    ...
    {
      let val = v8::String::new_external_onebyte_static(scope, env!("TARGET").as_bytes()).unwrap();
      array.set_index(scope, 11, val.into());
    }

    {
      let val = v8::String::new_from_one_byte(scope, deno_core::v8_version().as_bytes(), v8::NewStringType::Normal).unwrap();
      array.set_index(scope, 12, val.into());
    }

    {
      let val = v8::String::new_from_one_byte(scope, self.user_agent.as_bytes(), v8::NewStringType::Normal).unwrap();
