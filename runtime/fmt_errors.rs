...
pub fn format_location(frame: &JsStackFrame) -> String {
  let _internal = frame.file_name.as_ref().map(|f| f.starts_with("ext:")).unwrap_or(false);
  if frame.is_native {
...
fn format_frame(frame: &JsStackFrame) -> String {
  ...
  if frame.is_promise_all {
    result += &italic_bold(&format!("Promise.all (index {})", frame.promise_index.unwrap_or_default())).to_string();
    return result;
  }
  if is_method_call {
...
fn format_maybe_source_line(source_line: Option<&str>, column_number: Option<i64>, is_error: bool, level: usize) -> String {
...
fn find_recursive_cause(js_error: &JsError) -> Option<ErrorReference> {
  ...
  while let Some(cause) = &current_error.cause {
    ...
    if let Some(seen) = history.iter().find(|&el| errors_are_equal_without_cause(el, cause.as_ref())) {
...
fn format_aggregated_error(aggregated_errors: &Vec<JsError>, circular_reference_index: usize) -> String {
  let mut s = String::new();
...
fn format_js_error_inner(js_error: &JsError, circular: Option<IndexedErrorReference>, include_source_code: bool) -> String {
  let mut s = String::new();
  ...
  if let Some(aggregated) = &js_error.aggregated {
    let aggregated_message = format_aggregated_error(aggregated, circular.as_ref().map(|circular| circular.index).unwrap_or(0));
  ...
  s.push_str(&format_maybe_source_line(
    if include_source_code { js_error.source_line.as_deref() } else { None },
    column_number,
    true,
    0,
  ));
...
pub fn format_js_error(js_error: &JsError) -> String {
  let circular = find_recursive_cause(js_error).map(|reference| IndexedErrorReference { reference, index: 1 });
