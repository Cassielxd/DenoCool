...
/// Websocket service that is used to proxy connections from
...
impl InspectorServer {
  pub fn new(host: SocketAddr, name: &'static str) -> Self {
    ...
    let thread_handle = thread::spawn(move || {
      ...
      local.block_on(&rt, server(host, register_inspector_rx, shutdown_server_rx, name))
    });
  ...
  pub fn register_inspector(&self, module_url: String, js_runtime: &mut JsRuntime, wait_for_session: bool) {
    let inspector_rc = js_runtime.inspector();
    let mut inspector = inspector_rc.borrow_mut();
    let session_sender = inspector.get_session_sender();
    let deregister_rx = inspector.add_deregister_handler();
    let info = InspectorInfo::new(self.host, session_sender, deregister_rx, module_url, wait_for_session);
...
fn handle_ws_request(
  ...
  let maybe_uuid = req.uri().path().strip_prefix("/ws/").and_then(|s| Uuid::parse_str(s).ok());
  ...
  spawn(async move {
    let websocket = if let Ok(w) = fut.await {
      ...
      eprintln!("Inspector service failed to upgrade to WS connection");
      return;
    };
...
fn handle_json_request(inspector_map: Rc<RefCell<HashMap<Uuid, InspectorInfo>>>) -> http::Result<http::Response<hyper::Body>> {
  let data = inspector_map.borrow().values().map(|info| info.get_json_metadata()).collect::<Vec<_>>();
...
fn handle_json_version_request(version_response: Value) -> http::Result<http::Response<hyper::Body>> {
  http::Response::builder()
    .status(http::StatusCode::OK)
    .header(http::header::CONTENT_TYPE, "application/json")
...
async fn server(host: SocketAddr, register_inspector_rx: UnboundedReceiver<InspectorInfo>, shutdown_server_rx: oneshot::Receiver<()>, name: &str) {
  ...
  let mut register_inspector_handler = pin!(register_inspector_rx
    .map(|info| {
      eprintln!("调试 服务是 {}", info.get_websocket_debugger_url());
      eprintln!("可以使用 chrome://inspect 进行调试.");
  ...
  let make_svc = hyper::service::make_service_fn(|_| {
    ...
    future::ok::<_, Infallible>(hyper::service::service_fn(move |req: http::Request<hyper::Body>| {
      future::ready({
        match (req.method(), req.uri().path()) {
          (&http::Method::GET, path) if path.starts_with("/ws/") => handle_ws_request(req, Rc::clone(&inspector_map)),
          (&http::Method::GET, "/json/version") => handle_json_version_request(json_version_response.clone()),
          (&http::Method::GET, "/json") => handle_json_request(Rc::clone(&inspector_map)),
          (&http::Method::GET, "/json/list") => handle_json_request(Rc::clone(&inspector_map)),
          _ => http::Response::builder().status(http::StatusCode::NOT_FOUND).body("Not Found".into()),
        }
      })
    }))
  });

  // Create the service manually so it can use the Local Executor
  ...
  .unwrap_or_else(|err| {
    eprintln!("Cannot start inspector service: {err}.");
    process::exit(1);
  })
  .fuse());
...
/// Inspector information that is sent from the isolate thread to the service
...
impl InspectorInfo {
  ...
  fn get_title(&self) -> String {
    format!(
      "deno{} [pid: {}]",
      self.thread_name.as_ref().map(|n| format!(" - {n}")).unwrap_or_default(),
      process::id(),
    )
  }
}
