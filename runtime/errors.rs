...
fn get_request_error_class(error: &reqwest::Error) -> &'static str {
  error
    .source()
    .and_then(|inner_err| {
      (inner_err.downcast_ref::<io::Error>().map(get_io_error_class))
        .or_else(|| inner_err.downcast_ref::<serde_json::error::Error>().map(get_serde_json_error_class))
        .or_else(|| inner_err.downcast_ref::<url::ParseError>().map(get_url_parse_error_class))
    })
    .unwrap_or("Http")
}
...
pub fn get_error_class_name(e: &AnyError) -> Option<&'static str> {
  deno_core::error::get_custom_error_class(e)
    ...
    .or_else(|| e.downcast_ref::<Arc<hyper::Error>>().map(|e| get_hyper_error_class(e)))
    .or_else(|| {
