...
pub fn init_runtime_module_map(js_runtime: &mut JsRuntime) {
  js_runtime.clear_module_map(deno_node::SUPPORTED_BUILTIN_NODE_MODULES.iter().map(|p| (p.ext_specifier, p.specifier)));
}
...
impl Default for WorkerOptions {
  fn default() -> Self {
    Self {
      ...
      web_worker_pre_execute_module_cb: Arc::new(|_| unimplemented!("web workers are not supported")),
...
impl MainWorker {
  ...
  pub fn from_options(main_module: ModuleSpecifier, permissions: PermissionsContainer, mut options: WorkerOptions) -> Self {
    ...
    let mut extensions = vec![
      ...
      deno_web::deno_web::init_ops::<PermissionsContainer>(options.blob_store.clone(), options.bootstrap.location.clone()),
      ...
      deno_broadcast_channel::deno_broadcast_channel::init_ops(options.broadcast_channel.clone(), unstable),
      ...
      deno_kv::deno_kv::init_ops(SqliteDbHandler::<PermissionsContainer>::new(options.origin_storage_dir.clone()), unstable),
      deno_napi::deno_napi::init_ops::<PermissionsContainer>(),
      deno_http::deno_http::init_ops::<DefaultHttpPropertyExtractor>(),
      deno_io::deno_io::init_ops(Some(options.stdio)),
      deno_fs::deno_fs::init_ops::<PermissionsContainer>(unstable, options.fs.clone()),
      deno_node::deno_node::init_ops::<PermissionsContainer>(options.npm_resolver, options.fs),
      // Ops from this crate
    ...
    let startup_snapshot = options.startup_snapshot.unwrap_or_else(crate::js::deno_isolate_init);
    ...
    let bootstrap_fn_global = {
      ...
      let bootstrap_ns: v8::Local<v8::Object> = global_obj.get(scope, bootstrap_str.into()).unwrap().try_into().unwrap();
  ...
  pub fn bootstrap(&mut self, options: &BootstrapOptions) {
    ...
    bootstrap_fn.call(scope, undefined.into(), &[args.into()]).unwrap();
  }

  /// See [JsRuntime::execute_script](deno_core::JsRuntime::execute_script)
  pub fn execute_script(&mut self, script_name: &'static str, source_code: ModuleCode) -> Result<v8::Global<v8::Value>, AnyError> {
  ...
  pub async fn preload_main_module(&mut self, module_specifier: &ModuleSpecifier) -> Result<ModuleId, AnyError> {
    self.js_runtime.load_main_module(module_specifier, None).await
  }

  /// Loads and instantiates specified JavaScript module as "side" module.
  pub async fn preload_side_module(&mut self, module_specifier: &ModuleSpecifier) -> Result<ModuleId, AnyError> {
    self.js_runtime.load_side_module(module_specifier, None).await
  }
  ...
  fn wait_for_inspector_session(&mut self) {
    if self.should_break_on_first_statement {
      self.js_runtime.inspector().borrow_mut().wait_for_session_and_break_on_next_statement();
  ...
  pub fn poll_event_loop(&mut self, cx: &mut Context, wait_for_inspector: bool) -> Poll<Result<(), AnyError>> {
  ...
  pub async fn with_event_loop<'a, T>(&mut self, mut fut: Pin<Box<dyn Future<Output = T> + 'a>>) -> T {
    loop {
      tokio::select! {
        biased;
        result = &mut fut => {
