...
impl PermissionState {
  ...
  fn log_perm_access(name: &str, info: impl FnOnce() -> Option<String>) {
    ...
    if *DEBUG_LOG_ENABLED {
      log::debug!(
        "{}",
        colors::bold(&format!("{}️  Granted {}", PERMISSION_EMOJI, Self::fmt_access(name, info)))
      );
    }
  }

  fn fmt_access(name: &str, info: impl FnOnce() -> Option<String>) -> String {
    format!("{} access{}", name, info().map(|info| { format!(" to {info}") }).unwrap_or_default(),)
  }

  fn error(name: &str, info: impl FnOnce() -> Option<String>) -> AnyError {
    custom_error(
      "PermissionDenied",
      format!("Requires {}, run again with the --allow-{} flag", Self::fmt_access(name, info), name),
    )
  }

  /// Check the permission state. bool is whether a prompt was issued.
  #[inline]
  fn check(self, name: &str, api_name: Option<&str>, info: Option<&str>, prompt: bool) -> (Result<(), AnyError>, bool, bool) {
    self.check2(name, api_name, || info.map(|s| s.to_string()), prompt)
  }

  #[inline]
  fn check2(self, name: &str, api_name: Option<&str>, info: impl Fn() -> Option<String>, prompt: bool) -> (Result<(), AnyError>, bool, bool) {
    match self {
      ...
      PermissionState::Prompt if prompt => {
        let msg = format!("{} access{}", name, info().map(|info| { format!(" to {info}") }).unwrap_or_default(),);
...
impl UnitPermission {
  ...
  pub fn check(&mut self) -> Result<(), AnyError> {
    let (result, prompted, _is_allow_all) = self.state.check(self.name, None, None, self.prompt);
    if prompted {
      if result.is_ok() {
        self.state = PermissionState::Granted;
      } else {
...
pub fn parse_sys_kind(kind: &str) -> Result<&str, AnyError> {
  match kind {
    "hostname" | "osRelease" | "osUptime" | "loadavg" | "networkInterfaces" | "systemMemoryInfo" | "uid" | "gid" => Ok(kind),
...
impl UnaryPermission<ReadDescriptor> {
  pub fn query(&self, path: Option<&Path>) -> PermissionState {
      ...
      }
    {
      ...
      || match path.as_ref() {
        None => false,
        Some(path) => self.granted_list.iter().any(|path_| path.starts_with(&path_.0)),
      }
    {
      PermissionState::Granted
    } else {
  ...
  pub fn request(&mut self, path: Option<&Path>) -> PermissionState {
    if let Some(path) = path {
      ...
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("read access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
  ...
  pub fn revoke(&mut self, path: Option<&Path>) -> PermissionState {
    if let Some(path) = path {
      ...
      self.granted_list.retain(|path_| !path.starts_with(&path_.0));
    } else {
      self.granted_list.clear();
    }
    if self.global_state == PermissionState::Granted {
  ...
  pub fn check(&mut self, path: &Path, api_name: Option<&str>) -> Result<(), AnyError> {
    let (result, prompted, is_allow_all) =
      self
        .query(Some(path))
        .check2(self.name, api_name, || Some(format!("\"{}\"", path.to_path_buf().display())), self.prompt);
    if prompted {
  ...
  pub fn check_blind(&mut self, path: &Path, display: &str, api_name: &str) -> Result<(), AnyError> {
    let resolved_path = resolve_from_cwd(path)?;
    let (result, prompted, is_allow_all) =
      self
        .query(Some(&resolved_path))
        .check(self.name, Some(api_name), Some(&format!("<{display}>")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
  ...
  pub fn check_all(&mut self, api_name: Option<&str>) -> Result<(), AnyError> {
    let (result, prompted, _) = self.query(None).check(self.name, api_name, Some("all"), self.prompt);
    if prompted {
      if result.is_ok() {
        self.global_state = PermissionState::Granted;
      } else {
...
impl UnaryPermission<WriteDescriptor> {
  ...
  pub fn request(&mut self, path: Option<&Path>) -> PermissionState {
    if let Some(path) = path {
      ...
      let state = self.query(None);
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("write access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
...
impl UnaryPermission<NetDescriptor> {
  pub fn query<T: AsRef<str>>(&self, host: Option<&(T, Option<u16>)>) -> PermissionState {
      ...
      }
    {
      ...
      || match host.as_ref() {
        None => false,
        Some(host) => {
          self.granted_list.contains(&NetDescriptor::new(&&(host.0.as_ref().to_string(), None)))
  ...
  pub fn request<T: AsRef<str>>(&mut self, host: Option<&(T, Option<u16>)>) -> PermissionState {
    if let Some(host) = host {
      ...
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("network access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
  ...
  pub fn revoke<T: AsRef<str>>(&mut self, host: Option<&(T, Option<u16>)>) -> PermissionState {
    if let Some(host) = host {
      if host.1.is_some() {
        self.granted_list.remove(&NetDescriptor(host.0.as_ref().to_string(), host.1));
      }
      self.granted_list.remove(&NetDescriptor(host.0.as_ref().to_string(), None));
    } else {
      self.granted_list.clear();
    }
    if self.global_state == PermissionState::Granted {
  ...
  pub fn check<T: AsRef<str>>(&mut self, host: &(T, Option<u16>), api_name: Option<&str>) -> Result<(), AnyError> {
    let new_host = NetDescriptor::new(&host);
    let (result, prompted, is_allow_all) = self
      .query(Some(host))
      .check(self.name, api_name, Some(&format!("\"{new_host}\"")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
  ...
  pub fn check_url(&mut self, url: &url::Url, api_name: Option<&str>) -> Result<(), AnyError> {
    ...
    let (result, prompted, is_allow_all) = self
      .query(Some(host))
      .check(self.name, api_name, Some(&format!("\"{display_host}\"")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
  ...
  pub fn check_all(&mut self) -> Result<(), AnyError> {
    let (result, prompted, _) = self.query::<&str>(None).check(self.name, None, Some("all"), self.prompt);
    if prompted {
      if result.is_ok() {
        self.global_state = PermissionState::Granted;
      } else {
...
impl UnaryPermission<EnvDescriptor> {
  ...
  pub fn request(&mut self, env: Option<&str>) -> PermissionState {
    if let Some(env) = env {
      ...
      if state == PermissionState::Prompt {
        match permission_prompt(&format!("env access to \"{env}\""), self.name, Some("Deno.permissions.query()"), true) {
          PromptResponse::Allow => {
      ...
      let state = self.query(None);
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("env access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
  ...
  pub fn check(&mut self, env: &str) -> Result<(), AnyError> {
    let (result, prompted, is_allow_all) = self.query(Some(env)).check(self.name, None, Some(&format!("\"{env}\"")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
  ...
  pub fn check_all(&mut self) -> Result<(), AnyError> {
    let (result, prompted, _) = self.query(None).check(self.name, None, Some("all"), self.prompt);
    if prompted {
      if result.is_ok() {
        self.global_state = PermissionState::Granted;
      } else {
...
impl UnaryPermission<SysDescriptor> {
  pub fn query(&self, kind: Option<&str>) -> PermissionState {
    if self.global_state == PermissionState::Denied
      && match kind {
        None => true,
        Some(kind) => self.denied_list.contains(&SysDescriptor(kind.to_string())),
      }
    {
      PermissionState::Denied
    } else if self.global_state == PermissionState::Granted
      || match kind {
        None => false,
        Some(kind) => self.granted_list.contains(&SysDescriptor(kind.to_string())),
      }
    {
      PermissionState::Granted
    } else {
  ...
  pub fn request(&mut self, kind: Option<&str>) -> PermissionState {
    ...
    if let Some(kind) = kind {
      ...
      match permission_prompt(&format!("sys access to \"{kind}\""), self.name, Some("Deno.permissions.query()"), true) {
        PromptResponse::Allow => {
      ...
      if PromptResponse::Allow == permission_prompt("sys access", self.name, Some("Deno.permissions.query()"), true) {
        self.global_state = PermissionState::Granted;
      } else {
        self.granted_list.clear();
        self.global_state = PermissionState::Denied;
  ...
  pub fn check(&mut self, kind: &str, api_name: Option<&str>) -> Result<(), AnyError> {
    let (result, prompted, is_allow_all) = self
      .query(Some(kind))
      .check(self.name, api_name, Some(&format!("\"{kind}\"")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
  ...
  pub fn check_all(&mut self) -> Result<(), AnyError> {
    let (result, prompted, _is_allow_all) = self.query(None).check(self.name, None, Some("all"), self.prompt);
    if prompted {
      if result.is_ok() {
        self.global_state = PermissionState::Granted;
      } else {
...
impl UnaryPermission<RunDescriptor> {
  pub fn query(&self, cmd: Option<&str>) -> PermissionState {
      ...
      }
    {
      ...
      || match cmd {
        None => false,
        Some(cmd) => self.granted_list.contains(&RunDescriptor::from_str(cmd).unwrap()),
      }
    {
      PermissionState::Granted
    } else {
  ...
  pub fn request(&mut self, cmd: Option<&str>) -> PermissionState {
    if let Some(cmd) = cmd {
      ...
      if state == PermissionState::Prompt {
        match permission_prompt(&format!("run access to \"{cmd}\""), self.name, Some("Deno.permissions.query()"), true) {
          PromptResponse::Allow => {
            self.granted_list.insert(RunDescriptor::from_str(cmd).unwrap());
            PermissionState::Granted
          }
          PromptResponse::Deny => {
            self.denied_list.insert(RunDescriptor::from_str(cmd).unwrap());
            self.global_state = PermissionState::Denied;
            PermissionState::Denied
          }
          PromptResponse::AllowAll => {
      ...
      let state = self.query(None);
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("run access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
  ...
  pub fn revoke(&mut self, cmd: Option<&str>) -> PermissionState {
    if let Some(cmd) = cmd {
      self.granted_list.remove(&RunDescriptor::from_str(cmd).unwrap());
    } else {
      self.granted_list.clear();
    }
    if self.global_state == PermissionState::Granted {
  ...
  pub fn check(&mut self, cmd: &str, api_name: Option<&str>) -> Result<(), AnyError> {
    let (result, prompted, is_allow_all) = self.query(Some(cmd)).check(self.name, api_name, Some(&format!("\"{cmd}\"")), self.prompt);
    if prompted {
      if result.is_ok() {
        if is_allow_all {
          self.granted_list.clear();
...
impl UnaryPermission<FfiDescriptor> {
  ...
  pub fn request(&mut self, path: Option<&Path>) -> PermissionState {
    if let Some(path) = path {
      ...
      let state = self.query(None);
      if state == PermissionState::Prompt {
        if PromptResponse::Allow == permission_prompt("ffi access", self.name, Some("Deno.permissions.query()"), true) {
          self.granted_list.clear();
          self.global_state = PermissionState::Granted;
          PermissionState::Granted
        } else {
  ...
  pub fn check(&mut self, path: Option<&Path>) -> Result<(), AnyError> {
    if let Some(path) = path {
      ...
      let (result, prompted, is_allow_all) =
        self
          .query(Some(&resolved_path))
          .check(self.name, None, Some(&format!("\"{}\"", display_path.display())), self.prompt);
      ...
      let (result, prompted, _) = self.query(None).check(self.name, None, None, self.prompt);
...
impl Permissions {
  pub fn new_read(state: &Option<Vec<PathBuf>>, prompt: bool) -> Result<UnaryPermission<ReadDescriptor>, AnyError> {
  ...
  pub fn new_write(state: &Option<Vec<PathBuf>>, prompt: bool) -> Result<UnaryPermission<WriteDescriptor>, AnyError> {
  ...
  pub fn new_net(state: &Option<Vec<String>>, prompt: bool) -> Result<UnaryPermission<NetDescriptor>, AnyError> {
  ...
  pub fn new_env(state: &Option<Vec<String>>, prompt: bool) -> Result<UnaryPermission<EnvDescriptor>, AnyError> {
  ...
  pub fn new_sys(state: &Option<Vec<String>>, prompt: bool) -> Result<UnaryPermission<SysDescriptor>, AnyError> {
  ...
  pub fn new_run(state: &Option<Vec<String>>, prompt: bool) -> Result<UnaryPermission<RunDescriptor>, AnyError> {
  ...
  pub fn new_ffi(state: &Option<Vec<PathBuf>>, prompt: bool) -> Result<UnaryPermission<FfiDescriptor>, AnyError> {
  ...
  pub fn check_specifier(&mut self, specifier: &ModuleSpecifier) -> Result<(), AnyError> {
    match specifier.scheme() {
      "file" => match specifier.to_file_path() {
        ...
        Err(_) => Err(uri_error(format!("Invalid file path.\n  Specifier: {specifier}"))),
      },
      "data" => Ok(()),
      "blob" => Ok(()),
...
fn unit_permission_from_flag_bool(flag: bool, name: &'static str, description: &'static str, prompt: bool) -> UnitPermission {
  UnitPermission {
    name,
    description,
...
pub fn resolve_read_allowlist(allow: &Option<Vec<PathBuf>>) -> Result<HashSet<ReadDescriptor>, AnyError> {
  if let Some(v) = allow {
    v.iter()
      .map(|raw_path| {
...
pub fn resolve_write_allowlist(allow: &Option<Vec<PathBuf>>) -> Result<HashSet<WriteDescriptor>, AnyError> {
  if let Some(v) = allow {
    v.iter()
      .map(|raw_path| {
        if raw_path.as_os_str().is_empty() {
...
pub fn resolve_ffi_allowlist(allow: &Option<Vec<PathBuf>>) -> Result<HashSet<FfiDescriptor>, AnyError> {
  if let Some(v) = allow {
    v.iter()
      .map(|raw_path| {
        if raw_path.as_os_str().is_empty() {
...
fn escalation_error() -> AnyError {
  custom_error("PermissionDenied", "Can't escalate parent thread permissions")
}
        ...
        V: de::MapAccess<'de>,
      {
        ...
        while let Some((key, value)) = v.next_entry::<String, serde_json::Value>()? {
          if key == "env" {
            ...
            child_permissions_arg.env = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.env) {e}")))?;
          } else if key == "hrtime" {
            let arg = serde_json::from_value::<ChildUnitPermissionArg>(value);
            child_permissions_arg.hrtime = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.hrtime) {e}")))?;
          } else if key == "net" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.net = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.net) {e}")))?;
          } else if key == "ffi" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.ffi = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.ffi) {e}")))?;
          } else if key == "read" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.read = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.read) {e}")))?;
          } else if key == "run" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.run = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.run) {e}")))?;
          } else if key == "sys" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.sys = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.sys) {e}")))?;
          } else if key == "write" {
            let arg = serde_json::from_value::<ChildUnaryPermissionArg>(value);
            child_permissions_arg.write = arg.map_err(|e| de::Error::custom(format!("(deno.permissions.write) {e}")))?;
          } else {
...
pub fn create_child_permissions(main_perms: &mut Permissions, child_permissions_arg: ChildPermissionsArg) -> Result<Permissions, AnyError> {
  ...
  match child_permissions_arg.env {
    ...
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.env.granted_list = Permissions::new_env(&Some(granted_list), false)?.granted_list;
      if !worker_perms
        .env
        .granted_list
        .iter()
  ...
  match child_permissions_arg.sys {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.sys.granted_list = Permissions::new_sys(&Some(granted_list), false)?.granted_list;
      if !worker_perms
        .sys
        .granted_list
        .iter()
  ...
  match child_permissions_arg.net {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.net.granted_list = Permissions::new_net(&Some(granted_list), false)?.granted_list;
      if !worker_perms
        .net
        .granted_list
        .iter()
  ...
  match child_permissions_arg.ffi {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.ffi.granted_list = Permissions::new_ffi(&Some(granted_list.iter().map(PathBuf::from).collect()), false)?.granted_list;
      if !worker_perms
        .ffi
        .granted_list
        .iter()
  ...
  match child_permissions_arg.read {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.read.granted_list = Permissions::new_read(&Some(granted_list.iter().map(PathBuf::from).collect()), false)?.granted_list;
      if !worker_perms
        .read
        .granted_list
        .iter()
  ...
  match child_permissions_arg.run {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.run.granted_list = Permissions::new_run(&Some(granted_list), false)?.granted_list;
      if !worker_perms
        .run
        .granted_list
        .iter()
  ...
  match child_permissions_arg.write {
    ...
    ChildUnaryPermissionArg::NotGranted => {}
    ChildUnaryPermissionArg::GrantedList(granted_list) => {
      worker_perms.write.granted_list = Permissions::new_write(&Some(granted_list.iter().map(PathBuf::from).collect()), false)?.granted_list;
      if !worker_perms
        .write
        .granted_list
        .iter()
...
mod tests {
  ...
  fn check_paths() {
    ...
    assert!(perms.read.check(Path::new("/a/specific/dir/name"), None).is_ok());
    assert!(perms.write.check(Path::new("/a/specific/dir/name"), None).is_ok());
    assert!(perms.ffi.check(Some(Path::new("/a/specific/dir/name"))).is_ok());
    ...
    // Inside of /a/specific and /a/specific/dir/name
    assert!(perms.read.check(Path::new("/a/specific/dir/name/inner"), None).is_ok());
    assert!(perms.write.check(Path::new("/a/specific/dir/name/inner"), None).is_ok());
    assert!(perms.ffi.check(Some(Path::new("/a/specific/dir/name/inner"))).is_ok());
    ...
    assert!(perms.ffi.check(Some(Path::new("/a/specific/other/dir"))).is_ok());
    ...
    assert!(perms.read.check(Path::new("/b/c/sub/path/../path/."), None).is_ok());
    assert!(perms.write.check(Path::new("/b/c/sub/path/../path/."), None).is_ok());
    assert!(perms.ffi.check(Some(Path::new("/b/c/sub/path/../path/."))).is_ok());
  ...
  fn check_specifiers() {
    ...
    let mut fixtures = vec![
      (ModuleSpecifier::parse("http://localhost:4545/mod.ts").unwrap(), true),
      (ModuleSpecifier::parse("http://deno.land/x/mod.ts").unwrap(), false),
      (ModuleSpecifier::parse("data:text/plain,Hello%2C%20Deno!").unwrap(), true),
    ];

    if cfg!(target_os = "windows") {
      fixtures.push((ModuleSpecifier::parse("file:///C:/a/mod.ts").unwrap(), true));
      fixtures.push((ModuleSpecifier::parse("file:///C:/b/mod.ts").unwrap(), false));
    } else {
  ...
  fn check_invalid_specifiers() {
    ...
    for url in test_cases {
      assert!(perms.check_specifier(&ModuleSpecifier::parse(url).unwrap()).is_err());
    }
  }
  ...
  fn test_query() {
    ...
    let perms2 = Permissions {
      ...
      ffi: UnaryPermission {
        global_state: PermissionState::Prompt,
        ..Permissions::new_ffi(&Some(vec![PathBuf::from("/foo")]), false).unwrap()
      },
  ...
  fn test_revoke() {
    ...
    let mut perms = Permissions {
      read: UnaryPermission {
        global_state: PermissionState::Prompt,
        ..Permissions::new_read(&Some(vec![PathBuf::from("/foo"), PathBuf::from("/foo/baz")]), false).unwrap()
      },
      write: UnaryPermission {
        global_state: PermissionState::Prompt,
        ..Permissions::new_write(&Some(vec![PathBuf::from("/foo"), PathBuf::from("/foo/baz")]), false).unwrap()
      },
      ffi: UnaryPermission {
        global_state: PermissionState::Prompt,
        ..Permissions::new_ffi(&Some(vec![PathBuf::from("/foo"), PathBuf::from("/foo/baz")]), false).unwrap()
      },
      net: UnaryPermission {
        global_state: PermissionState::Prompt,
  ...
  fn test_create_child_permissions_with_inherited_denied_list() {
    ...
    let worker_perms = create_child_permissions(&mut main_perms.clone(), ChildPermissionsArg::none()).unwrap();
