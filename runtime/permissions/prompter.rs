...
static PERMISSION_PROMPTER: Lazy<Mutex<Box<dyn PermissionPrompter>>> = Lazy::new(|| Mutex::new(Box::new(TtyPrompter)));

static MAYBE_BEFORE_PROMPT_CALLBACK: Lazy<Mutex<Option<PromptCallback>>> = Lazy::new(|| Mutex::new(None));

static MAYBE_AFTER_PROMPT_CALLBACK: Lazy<Mutex<Option<PromptCallback>>> = Lazy::new(|| Mutex::new(None));

pub fn permission_prompt(message: &str, flag: &str, api_name: Option<&str>, is_unary: bool) -> PromptResponse {
  if let Some(before_callback) = MAYBE_BEFORE_PROMPT_CALLBACK.lock().as_mut() {
    before_callback();
  }
  let r = PERMISSION_PROMPTER.lock().prompt(message, flag, api_name, is_unary);
...
pub trait PermissionPrompter: Send + Sync {
  fn prompt(&mut self, message: &str, name: &str, api_name: Option<&str>, is_unary: bool) -> PromptResponse;
}
  ...
  fn prompt(&mut self, message: &str, name: &str, api_name: Option<&str>, is_unary: bool) -> PromptResponse {
    ...
    fn clear_stdin() -> Result<(), AnyError> {
      ...
      unsafe fn flush_input_buffer(stdin: HANDLE) -> Result<(), AnyError> {
        ...
        if success != TRUE {
          bail!("Could not flush the console input buffer: {}", std::io::Error::last_os_error())
        }
        Ok(())
      }
      ...
      unsafe fn emulate_enter_key_press(stdin: HANDLE) -> Result<(), AnyError> {
        ...
        input_record.Event.KeyEvent_mut().wVirtualScanCode = MapVirtualKeyW(VK_RETURN as UINT, MAPVK_VK_TO_VSC) as WORD;
        *input_record.Event.KeyEvent_mut().uChar.UnicodeChar_mut() = '\r' as WCHAR;
        ...
        if success != TRUE {
          bail!("Could not emulate enter key press: {}", std::io::Error::last_os_error());
        }
        Ok(())
      }
      ...
      unsafe fn is_input_buffer_empty(stdin: HANDLE) -> Result<bool, AnyError> {
        ...
        if success != TRUE {
          bail!("Could not peek the console input buffer: {}", std::io::Error::last_os_error())
        }
        Ok(events_read == 0)
      }
