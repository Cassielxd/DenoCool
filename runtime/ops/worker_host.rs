...
pub type CreateWebWorkerCb = dyn Fn(CreateWebWorkerArgs) -> (WebWorker, SendableWebWorkerHandle) + Sync + Send;

pub type WorkerEventCb = dyn Fn(WebWorker) -> LocalFutureObj<'static, Result<WebWorker, AnyError>> + Sync + Send;
...
/// 创建一个新的  woker 并持有该worker的handle
#[op]
fn op_create_worker(state: &mut OpState, args: CreateWorkerArgs) -> Result<WorkerId, AnyError> {
  ...
  if let WebWorkerType::Classic = worker_type {
    if let TestingFeaturesEnabled(false) = state.borrow() {
      return Err(deno_webstorage::DomExceptionNotSupportedError::new("Classic workers are not supported.").into());
    }
  }
  ...
  let (handle_sender, handle_receiver) = std::sync::mpsc::sync_channel::<Result<SendableWebWorkerHandle, AnyError>>(1);
...
/// 关闭worker 通信的通道 如果两个都关了
/// 直接移除 WorkersTable里的 子进程
...
/// 接收控制worker进程的 event事件
#[op]
async fn op_host_recv_ctrl(state: Rc<RefCell<OpState>>, id: WorkerId) -> Result<WorkerControlEvent, AnyError> {
  ...
  let maybe_event = worker_handle.get_control_event().or_cancel(cancel_handle).await;
  match maybe_event {
    Ok(Ok(Some(event))) => {
...
//主进程接收消息
#[op]
async fn op_host_recv_message(state: Rc<RefCell<OpState>>, id: WorkerId) -> Result<Option<JsMessageData>, AnyError> {
  let (worker_handle, cancel_handle) = {
    let s = state.borrow();
  ...
  let ret = worker_handle.port.recv(state.clone()).or_cancel(cancel_handle).await;
  match ret {
    Ok(Ok(ret)) => {
      if ret.is_none() {
...
/// 主进程向worker 发送消息
#[op]
fn op_host_post_message(state: &mut OpState, id: WorkerId, data: JsMessageData) -> Result<(), AnyError> {
