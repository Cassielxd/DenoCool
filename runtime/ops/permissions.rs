...
pub fn op_query_permission(state: &mut OpState, args: PermissionArgs) -> Result<String, AnyError> {
  ...
  let perm = match args.name.as_ref() {
    ...
    "sys" => permissions.sys.query(args.kind.as_deref().map(parse_sys_kind).transpose()?),
    ...
    n => return Err(custom_error("ReferenceError", format!("No such permission name: {n}"))),
  };
  Ok(perm.to_string())
}
...
pub fn op_revoke_permission(state: &mut OpState, args: PermissionArgs) -> Result<String, AnyError> {
  ...
  let perm = match args.name.as_ref() {
    ...
    "sys" => permissions.sys.revoke(args.kind.as_deref().map(parse_sys_kind).transpose()?),
...
#[op]
pub fn op_request_permission(state: &mut OpState, args: PermissionArgs) -> Result<String, AnyError> {
  let mut permissions = state.borrow_mut::<PermissionsContainer>().0.lock();
  let path = args.path.as_deref();
  let perm = match args.name.as_ref() {
    ...
    "sys" => permissions.sys.request(args.kind.as_deref().map(parse_sys_kind).transpose()?),
...
fn parse_host(host_str: &str) -> Result<(String, Option<u16>), AnyError> {
  let url = url::Url::parse(&format!("http://{host_str}/")).map_err(|_| uri_error("Invalid host"))?;
  if url.path() != "/" {
