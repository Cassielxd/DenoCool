    ...
    D: serde::Deserializer<'de>,
  {
    ...
    match value {
      Value::String(val) => match val.as_str() {
        ...
        val => Err(serde::de::Error::unknown_variant(val, &["inherit", "piped", "null"])),
      },
      Value::Number(val) => match val.as_u64() {
        Some(val) if val <= ResourceId::MAX as u64 => Ok(StdioOrRid::Rid(val as ResourceId)),
        _ => Err(serde::de::Error::custom("Expected a positive integer")),
      },
      _ => Err(serde::de::Error::custom(r#"Expected a resource id, "inherit", "piped", or "null""#)),
    }
  }
}
...
impl StdioOrRid {
  pub fn as_stdio(&self, state: &mut OpState) -> Result<std::process::Stdio, AnyError> {
    match &self {
      ...
      StdioOrRid::Rid(rid) => FileResource::with_file(state, *rid, |file| Ok(file.as_stdio()?)),
    }
  }
}
...
fn create_command(state: &mut OpState, args: SpawnArgs, api_name: &str) -> Result<std::process::Command, AnyError> {
  state.borrow_mut::<PermissionsContainer>().check_run(&args.cmd, api_name)?;
...
fn spawn_child(state: &mut OpState, command: std::process::Command) -> Result<Child, AnyError> {
  ...
  let stdin_rid = child.stdin.take().map(|stdin| state.resource_table.add(ChildStdinResource::from(stdin)));
  ...
  let child_rid = state.resource_table.add(ChildResource(RefCell::new(child), pid));
...
#[op]
fn op_spawn_child(state: &mut OpState, args: SpawnArgs, api_name: String) -> Result<Child, AnyError> {
  let command = create_command(state, args, &api_name)?;
  spawn_child(state, command)
}

#[op]
async fn op_spawn_wait(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<ChildStatus, AnyError> {
  #![allow(clippy::await_holding_refcell_ref)]
  let resource = state.borrow_mut().resource_table.get::<ChildResource>(rid)?;
  let result = resource.0.try_borrow_mut()?.wait().await?.try_into();
  state.borrow_mut().resource_table.close(rid).expect("shouldn't have closed until now");
  result
}
...
fn op_spawn_sync(state: &mut OpState, args: SpawnArgs) -> Result<SpawnOutput, AnyError> {
  ...
  let output = create_command(state, args, "Deno.Command().outputSync()")?.output()?;

  Ok(SpawnOutput {
    status: output.status.try_into()?,
    stdout: if stdout { Some(output.stdout.into()) } else { None },
    stderr: if stderr { Some(output.stderr.into()) } else { None },
  })
}

#[op]
fn op_spawn_kill(state: &mut OpState, rid: ResourceId, signal: String) -> Result<(), AnyError> {
...
mod deprecated {
  ...
  fn op_run(state: &mut OpState, run_args: RunArgs) -> Result<RunInfo, AnyError> {
    ...
    state.borrow_mut::<PermissionsContainer>().check_run(&args[0], "Deno.run()")?;
    ...
    let stdin_rid = match child.stdin.take() {
      Some(child_stdin) => {
        let rid = state.resource_table.add(ChildStdinResource::from(child_stdin));
        Some(rid)
      }
      None => None,
    };
    ...
    let stdout_rid = match child.stdout.take() {
      Some(child_stdout) => {
        let rid = state.resource_table.add(ChildStdoutResource::from(child_stdout));
        Some(rid)
      }
      None => None,
    };
    ...
    let stderr_rid = match child.stderr.take() {
      Some(child_stderr) => {
        let rid = state.resource_table.add(ChildStderrResource::from(child_stderr));
        Some(rid)
      }
      None => None,
    };
  ...
  #[op]
  async fn op_run_status(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<ProcessStatus, AnyError> {
    let resource = state.borrow_mut().resource_table.get::<ChildResource>(rid)?;
  ...
  #[op]
  fn op_kill(state: &mut OpState, pid: i32, signal: String, api_name: String) -> Result<(), AnyError> {
    state.borrow_mut::<PermissionsContainer>().check_run_all(&api_name)?;
    kill(pid, &signal)?;
    Ok(())
  }
}
