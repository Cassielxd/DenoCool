...
fn op_http_start(state: &mut OpState, tcp_stream_rid: ResourceId) -> Result<ResourceId, AnyError> {
  if let Ok(resource_rc) = state.resource_table.take::<TcpStreamResource>(tcp_stream_rid) {
    ...
    // process of starting a HTTP service on top of this TCP connection, so we just return a bad
  ...
  if let Ok(resource_rc) = state.resource_table.take::<TlsStreamResource>(tcp_stream_rid) {
    ...
    // process of starting a HTTP service on top of this TLS connection, so we just return a bad
    // resource error. See also: https://github.com/denoland/deno/pull/16242
    let resource = Rc::try_unwrap(resource_rc).map_err(|_| bad_resource("TLS stream is currently in use"))?;
    let (read_half, write_half) = resource.into_inner();
  ...
  #[cfg(unix)]
  if let Ok(resource_rc) = state.resource_table.take::<deno_net::io::UnixStreamResource>(tcp_stream_rid) {
    ...
    // process of starting a HTTP service on top of this UNIX socket, so we just return a bad
    // resource error. See also: https://github.com/denoland/deno/pull/16242
    let resource = Rc::try_unwrap(resource_rc).map_err(|_| bad_resource("UNIX stream is currently in use"))?;
    let (read_half, write_half) = resource.into_inner();
...
#[op]
async fn op_http_upgrade(state: Rc<RefCell<OpState>>, rid: ResourceId, _: ()) -> Result<HttpUpgradeResult, AnyError> {
  ...
  let request = match &mut *rd {
    ...
    _ => return Err(custom_error("Http", "cannot upgrade because request body was used")),
  };

  let transport = hyper::upgrade::on(request).await?;
  let transport = match transport.downcast::<TcpStream>() {
    Ok(Parts {
      io: tcp_stream, read_buf, ..
    }) => {
      return Ok(HttpUpgradeResult {
        conn_type: "tcp",
        conn_rid: state.borrow_mut().resource_table.add(TcpStreamResource::new(tcp_stream.into_split())),
  ...
  let transport = match transport.downcast::<UnixStream>() {
    Ok(Parts {
      io: unix_stream, read_buf, ..
    }) => {
      return Ok(HttpUpgradeResult {
        conn_type: "unix",
        conn_rid: state.borrow_mut().resource_table.add(UnixStreamResource::new(unix_stream.into_split())),
        read_buf: read_buf.to_vec().into(),
      });
    }
    Err(transport) => transport,
  ...
  match transport.downcast::<TlsStream>() {
    Ok(Parts {
      io: tls_stream, read_buf, ..
    }) => Ok(HttpUpgradeResult {
      conn_type: "tls",
      conn_rid: state.borrow_mut().resource_table.add(TlsStreamResource::new(tls_stream.into_split())),
      read_buf: read_buf.to_vec().into(),
    }),
    Err(_) => Err(custom_error("Http", "encountered unsupported transport while upgrading")),
  }
}
