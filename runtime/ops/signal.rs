...
pub fn signal_str_to_int(s: &str) -> Result<libc::c_int, AnyError> {
  match s {
    "SIGINT" => Ok(2),
    "SIGBREAK" => Ok(21),
    _ => Err(type_error("Windows only supports ctrl-c (SIGINT) and ctrl-break (SIGBREAK).")),
  }
}
...
fn op_signal_bind(state: &mut OpState, sig: &str) -> Result<ResourceId, AnyError> {
  ...
  if signal_hook_registry::FORBIDDEN.contains(&signo) {
    return Err(type_error(format!("Binding to signal '{sig}' is not allowed",)));
  }
...
fn op_signal_bind(state: &mut OpState, sig: &str) -> Result<ResourceId, AnyError> {
  ...
  let resource = SignalStreamResource {
    signal: AsyncRefCell::new(match signo {
      // SIGINT
      2 => ctrl_c().expect("There was an issue creating ctrl+c event stream.").into(),
      // SIGBREAK
      21 => ctrl_break().expect("There was an issue creating ctrl+break event stream.").into(),
      _ => unimplemented!(),
    }),
    cancel: Default::default(),
  };
...
async fn op_signal_poll(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<bool, AnyError> {
  let resource = state.borrow_mut().resource_table.get::<SignalStreamResource>(rid)?;
