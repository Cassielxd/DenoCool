...
#[cfg(windows)]
fn get_fd_from_resource(resource: Rc<FileResource>) -> Result<std::os::windows::io::RawHandle, AnyError> {
...
#[cfg(not(windows))]
fn get_fd_from_resource(resource: Rc<FileResource>) -> Result<std::os::unix::prelude::RawFd, AnyError> {
  ...
  #[cfg(unix)]
  {
    ...
    FileResource::with_resource(state, rid, move |resource| {
      ...
      if is_raw {
        ...
        raw.local_flags &= !(termios::LocalFlags::ECHO | termios::LocalFlags::ICANON | termios::LocalFlags::IEXTEN);
        if !cbreak {
...
fn op_console_size(state: &mut OpState, result: &mut [u32]) -> Result<(), AnyError> {
  fn check_console_size(state: &mut OpState, result: &mut [u32], rid: u32) -> Result<(), AnyError> {
    FileResource::with_resource(state, rid, move |resource| {
...
#[cfg(windows)]
fn console_size_from_fd(handle: std::os::windows::io::RawHandle) -> Result<ConsoleSize, std::io::Error> {
  // SAFETY: winapi calls
  unsafe {
...
mod tests {
  ...
  fn test_winos_raw_mode_transitions() {
    ...
    assert_eq!(known_off_modes[0], mode_raw_input_off(mode_raw_input_on(known_off_modes[0])));
    assert_eq!(known_off_modes[1], mode_raw_input_off(mode_raw_input_on(known_off_modes[1])));
  }
}
