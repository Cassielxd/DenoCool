...
fn op_fs_events_open(state: &mut OpState, args: OpenArgs) -> Result<ResourceId, AnyError> {
  ...
  for path in &args.paths {
    ...
    state.borrow_mut::<PermissionsContainer>().check_read(&path, "Deno.watchFs()")?;
...
#[op]
async fn op_fs_events_poll(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<Option<FsEvent>, AnyError> {
