...
#[op]
pub fn op_worker_sync_fetch(state: &mut OpState, scripts: Vec<String>, mut loose_mime_checks: bool) -> Result<Vec<SyncFetchScript>, AnyError> {
    ...
    let handles: Vec<_> = scripts
      ...
      .map(|script| -> JoinHandle<Result<SyncFetchScript, AnyError>> {
        ...
        runtime.spawn(async move {
          let script_url = Url::parse(&script).map_err(|_| type_error("Invalid script URL"))?;
          ...
          let (body, mime_type, res_url) = match script_url.scheme() {
            "http" | "https" => {
              ...
              let mime_type = resp.headers().get("Content-Type").and_then(|v| v.to_str().ok()).map(mime_type_essence);
            ...
            "data" => {
              let data_url = DataUrl::process(&script).map_err(|e| type_error(format!("{e:?}")))?;
          ...
          if !loose_mime_checks {
            ...
            match mime_type.as_deref() {
              ...
              None => return Err(DomExceptionNetworkError::new("Missing MIME type.").into()),
            }
          }
