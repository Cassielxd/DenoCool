...
fn create_handles(isolate_handle: v8::IsolateHandle, name: String, worker_type: WebWorkerType) -> (WebWorkerInternalHandle, SendableWebWorkerHandle) {
...
impl WebWorker {
  pub fn bootstrap_from_options(
    ...
    let (mut worker, handle) = Self::from_options(name, permissions, main_module, worker_id, options);
  ...
  pub fn from_options(
    ...
    let mut extensions: Vec<Extension> = vec![
      ...
      deno_web::deno_web::init_ops::<PermissionsContainer>(options.blob_store.clone(), Some(main_module.clone())),
      ...
      deno_kv::deno_kv::init_ops(SqliteDbHandler::<PermissionsContainer>::new(None), unstable),
      ...
      deno_permissions_web_worker::init_ops(permissions, unstable, enable_testing_features),
    ];
    ...
    let (internal_handle, external_handle) = {
      ...
      let (internal_handle, external_handle) = create_handles(handle, name.clone(), options.worker_type);
      let op_state = js_runtime.op_state();
    ...
    let bootstrap_fn_global = {
      ...
      let main_runtime_str = v8::String::new_external_onebyte_static(scope, b"workerRuntime").unwrap();
  ...
  pub fn bootstrap(&mut self, options: &BootstrapOptions) {
    ...
    // WebWorkers can have empty string as name.
    {
      ...
      let id_str: v8::Local<v8::Value> = v8::String::new(scope, &format!("{}", self.id)).unwrap().into();
      bootstrap_fn.call(scope, undefined.into(), &[args.into(), name_str, id_str]).unwrap();
    }
  ...
  fn poll_event_loop(&mut self, cx: &mut Context, wait_for_inspector: bool) -> Poll<Result<(), AnyError>> {
